...
// ─── INLINED MISSING HELPERS TO PREVENT BUILD CRASH ───
function safeText(val: any, fallback = '-') {
  if (val === undefined || val === null || String(val).trim() === '') return fallback;
  return String(val);
}

async function recordQrWorkflowStep(payload: any) {
  const dbPayload = {
    actor_staff_id: payload.actorStaffId,
    next_staff_id: payload.nextStaffId,
    shipment_id: payload.shipmentId,
    process_step: payload.processStep,
    territory_code: payload.territoryCode,
    scan_channel: payload.scanChannel,
    notes: payload.notes
  };
  const { error } = await supabase.from('qr_scan_events').insert(dbPayload);
  if (error) throw error;
}

async function acknowledgeWorkflow(id: string, status: string) {
  const { error } = await supabase.from('workflow_acknowledgements').update({ status }).eq('id', id);
  if (error) throw error;
}

async function bumpReminder(id: string) {
  const { data, error: fetchErr } = await supabase.from('workflow_acknowledgements').select('reminder_count').eq('id', id).single();
  if (fetchErr) throw fetchErr;
  const count = (data?.reminder_count || 0) + 1;
  const { error } = await supabase.from('workflow_acknowledgements').update({ reminder_count: count }).eq('id', id);
  if (error) throw error;
}
// ───────────────────────────────────────────────────────
...
const emptyMerchantForm = { merchant_id: '', merchant_code: '', merchant_name: '', phone_primary: '', address_line_1: '', township: '', city: 'Yangon', region_state: 'Yangon', business_type: '', payment_terms: 'COD', is_active: true };
const emptyTownshipForm = { township_code: '', township_name: '', township_mm: '', city: 'Yangon', region_state: 'Yangon', zone: 'Zone A', delivery_fee: '0', is_active: true };
const emptyStaffForm = { staff_code: '', full_name: '', staff_type: 'driver', role_name: 'driver', phone: '', email: '', branch_name: '', warehouse_id: '', is_active: true };
const emptyVehicleForm = { vehicle_code: '', registration_no: '', vehicle_type: 'motorbike', display_name: '', capacity_kg: '0', warehouse_id: '', status: 'available' };
const emptyAssetForm = { asset_code: '', asset_type: 'scanner', model_name: '', serial_no: '', status: 'available' };
const emptyAssignmentForm = { staff_id: '', asset_id: '', vehicle_id: '', notes: '', status: 'assigned' };
const emptyScanForm = { actor_staff_id: '', next_staff_id: '', shipment_id: '', process_step: 'handover', territory_code: '', scan_channel: 'qr_scanner', notes: '' };
...
export default function MasterDataPortal() {
  ...
  useEffect(() => { setView(currentViewFromPath(location.pathname)); }, [location.pathname]);

  async function loadData() {
    setLoading(true); setMessage('');
    try {
      const [ merchantData, townshipData, staffData, vehicleData, assetData, assignmentData, scanData, ackData, warehouseData, shipmentData ] = await Promise.all([
      ...
      setMerchantRows(merchantData); setTownshipRows(townshipData); setStaffRows(staffData); setVehicleRows(vehicleData); setAssetRows(assetData); setAssignmentRows(assignmentData); setScanRows(scanData); setAckRows(ackData); setWarehouseRows(warehouseData); setShipmentRows(shipmentData);
    } catch (e: any) { setMessage(e?.message || 'Unable to load master data.'); } finally { setLoading(false); }
  }
  ...
  async function insertRow(table: string, payload: any, successMessage: string) {
    ...
    try { const { error } = await supabase.from(table).insert(payload); if (error) throw error; setMessage(successMessage); await loadData(); return true; } 
    catch (e: any) { setMessage(e?.message || `Unable to insert into ${table}.`); return false; } finally { setSaving(false); }
  }
  ...
  async function updateRow(table: string, pkField: string, pkValue: any, payload: any, successMessage: string) {
    ...
    setSaving(true); setMessage('');
    try { const { error } = await supabase.from(table).update(payload).eq(pkField, pkValue); if (error) throw error; setMessage(successMessage); await loadData(); return true; } 
    catch (e: any) { setMessage(e?.message || `Unable to update ${table}.`); return false; } finally { setSaving(false); }
  }
  ...
  async function deleteRow(table: string, row: any, pkFields: string[], label: string) {
    ...
    setSaving(true); setMessage('');
    try { const { error } = await supabase.from(table).delete().eq(field, value); if (error) throw error; setMessage(`${label} deleted.`); await loadData(); } 
    catch (e: any) { setMessage(e?.message || `Unable to delete ${label}.`); } finally { setSaving(false); }
  }
  ...
  async function createScan() {
    ...
    } catch (e: any) { setMessage(e?.message || 'Unable to record scan step.'); } finally { setSaving(false); }
  }

  async function updateAck(id: string, status: 'accepted' | 'completed' | 'rejected') {
    setSaving(true); setMessage('');
    try { await acknowledgeWorkflow(id, status); setMessage(`Acknowledgement ${status}.`); await loadData(); } 
    catch (e: any) { setMessage(e?.message || 'Unable to update acknowledgement.'); } finally { setSaving(false); }
  }

  async function remindAck(id: string) {
    setSaving(true); setMessage('');
    try { await bumpReminder(id); setMessage('Reminder sent.'); await loadData(); } 
    catch (e: any) { setMessage(e?.message || 'Unable to remind acknowledgement.'); } finally { setSaving(false); }
  }
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-muted-foreground">
                        {shipmentMap.get(row.shipment_id)?.awb || shipmentMap.get(row.shipment_id)?.tracking_no || tt(language, 'No shipment', 'shipment မရှိ')} ·
      ...
      {view === 'acknowledgements' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>{tt(language, 'Accept, complete, reject, remind, or delete pending responsibility handoffs.', 'တာဝန်လွှဲပြောင်းမှုများကို manage လုပ်နိုင်သည်')}</CardDescription>
          </CardHeader>
              ...
              <div key={row.id} className="rounded-2xl border p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => deleteRow('workflow_acknowledgements', row, ['id'], 'acknowledgement')} disabled={saving}>
