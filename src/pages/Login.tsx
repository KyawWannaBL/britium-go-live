...
import {
  AlertCircle, ArrowLeft, ArrowRight, CheckCircle2, Copy, Download, Globe,
  Loader2, Lock, Mail, RefreshCw, ShieldCheck, UserPlus,
} from "lucide-react";
...
export default function LoginPage() {
  ...
  function FieldLabel({ text }: { text: string }) {
    return <label className="block text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">{text}</label>;
  }
      ...
      <div className="relative z-10 w-full max-w-md space-y-6 py-12">
        ...
        <Card className="bg-[#0B101B]/85 backdrop-blur-xl border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl">
          ...
          <CardContent className="p-7 md:p-8 space-y-5">
            {configMissing && <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-200 text-xs font-bold leading-relaxed">{t("Supabase configuration is missing. Please check environment variables.", "Supabase configuration မပြည့်စုံပါ။ Environment variables စစ်ဆေးပါ။")}</div>}
            ...
            {successMsg && <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-start gap-3 text-emerald-200"><CheckCircle2 className="h-5 w-5 shrink-0 mt-0.5" /><p className="text-xs font-bold leading-relaxed">{successMsg}</p></div>}
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="flex items-center justify-between text-xs">
                  <label className="flex items-center gap-2 text-slate-300">
                    <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} className="rounded border-white/20 bg-transparent" />
                    <span>{t("Remember me", "မှတ်ထားမည်")}</span>
                  </label>
                  <button type="button" onClick={() => { clearMessages(); setResetEmail(email); setView("forgot"); }} className="font-bold uppercase tracking-widest text-slate-400 hover:text-white">{t("Forgot Password?", "Password မေ့နေပါသလား?")}</button>
                </div>
                <Button type="submit" disabled={loading} className="w-full h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-black tracking-widest uppercase rounded-xl">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>{t("Login", "အကောင့်ဝင်မည်")}<ArrowRight className="ml-2 h-4 w-4" /></>}
                </Button>
              </form>
            )}

            {/* ... other views (forgot, force_change, mfa) are kept intact ... */}
            {view === "forgot" && (
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Mail className="absolute left-4 top-4 h-5 w-5 text-slate-400" />
                    <Input type="email" required value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} className="bg-black/40 border-white/10 text-white h-12 rounded-xl pl-12" />
                  </div>
                </div>
                <Button type="submit" disabled={loading} className="w-full h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-black tracking-widest uppercase rounded-xl">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t("Send Reset Link", "Reset Link ပို့မည်")}
                </Button>
                <Button type="button" variant="outline" onClick={() => { clearMessages(); setView("login"); }} className="w-full h-12 bg-black/30 border-white/10 text-white rounded-xl">
                  <ArrowLeft className="mr-2 h-4 w-4" />{t("Back to Login", "Login သို့ပြန်သွားမည်")}
                </Button>
              </form>
            )}
