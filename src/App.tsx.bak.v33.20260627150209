...
import BranchOfficeProcessPage from "@/pages/branch/BranchOfficeProcessPage";
import BranchOfficeSettlementPage from "@/pages/branch/BranchOfficeSettlementPage";
import { lazy, Suspense, useState, FormEvent, useEffect, type ReactNode } from "react";
import {
  HashRouter,
  Routes,
  Route,
  ...
  lazy(() =>
    importFunc().catch((err: any) => {
      ...
      return {
        default: () => (
          <div
            className={`flex h-screen w-full items-center justify-center bg-[#061524] text-[#ff4f86] text-[12px] font-bold tracking-widest uppercase ${GLOBAL_FONT}`}
          >
...
const CustomerServiceCommandCenterPage = safeLazy(() =>
  import("@/pages/CustomerServiceCommandCenterPage"),
);
const CustomerServicePortalPage = safeLazy(() =>
  import("@/pages/CustomerServicePortalPage"),
);
const DataEntryPage = safeLazy(() => import("@/pages/DataEntryPage"));
const DocumentPrintStudioPage = safeLazy(() =>
  import("@/pages/DocumentPrintStudioPage"),
);
...
const GoLiveTemplateCenterPage = safeLazy(() =>
  import("@/pages/GoLiveTemplateCenterPage"),
);
...
const SupervisorPickupAssignmentGoLivePage = safeLazy(() =>
  import("@/pages/SupervisorPickupAssignmentGoLivePage"),
);
const SupervisorPortalPage = safeLazy(() => import("@/pages/SupervisorPortalPage"));
const SupervisorWayplanReviewPage = safeLazy(() =>
  import("@/pages/SupervisorWayplanReviewPage"),
);
const TariffPage = safeLazy(() => import("@/pages/TariffPage"));
const UATGoLiveCommandCenterPage = safeLazy(() =>
  import("@/pages/UATGoLiveCommandCenterPage"),
);
const WarehouseOperationPage = safeLazy(() =>
  import("@/pages/WarehouseOperationPage"),
);
const WarehousePage = safeLazy(() => import("@/pages/WarehousePage"));
const WaybillStudioPage = safeLazy(() => import("@/pages/WaybillStudioPage"));
const WayplanCommandCenterPage = safeLazy(() =>
  import("@/pages/WayplanCommandCenterPage"),
);
const WorkforceCommissionPage = safeLazy(() =>
  import("@/pages/WorkforceCommissionPage"),
);
const DispatchCommandCenterPage = safeLazy(() =>
  import("@/pages/DispatchCommandCenterPage"),
);

// Reuse Biz Dev page until a separate BDM page exists.
const BusinessDevelopmentManagerPage = BizDevPage;

// ─── AUXILIARY PAGES ───
const DataEntryExcelRegisterPage = safeLazy(() =>
  import("@/pages/DataEntryExcelRegisterPage"),
);
const DataEntryPhotoCheckPage = safeLazy(() =>
  import("@/pages/DataEntryPhotoCheckPage"),
);
const DataEntrySynchronizedPage = safeLazy(() =>
  import("@/pages/DataEntrySynchronizedPage"),
);
const DataEntryUATUploadPage = safeLazy(() =>
  import("@/pages/DataEntryUATUploadPage"),
);
const DataEntryWaybillStudio = safeLazy(() =>
  import("@/pages/DataEntryWaybillStudio"),
);
const ProofGalleryPortalPage = safeLazy(() =>
  import("@/pages/ProofGalleryPortalPage"),
);
...
const WarehouseRegistrationTemplatePage = safeLazy(() =>
  import("@/pages/WarehouseRegistrationTemplatePage"),
);
const WarehouseUATUploadPage = safeLazy(() =>
  import("@/pages/WarehouseUATUploadPage"),
);
...
const WayplanTemplateGeneratorPage = safeLazy(() =>
  import("@/pages/WayplanTemplateGeneratorPage"),
);

// ─── LOADER ───
const PageLoader = () => (
  <div
    className={`flex h-screen w-full flex-col items-center justify-center bg-[#061524] gap-5 notranslate ${GLOBAL_FONT}`}
    translate="no"
  >
    <div className="w-14 h-14 border-4 border-[#1a3a5c] border-t-[#f6b84b] rounded-full animate-spin" />
...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    links: [
      { name: "Customer Service", path: "/customer-service", icon: HeadphonesIcon },
  ...
  },
  {
    ...
    links: [
      ...
      { name: "Warehouse Ops", path: "/warehouse-ops", icon: PackageSearch },
    ],
  },
  {
  ...
  },
  {
    ...
    links: [
      { name: "Rider Management", path: "/rider-management", icon: Bike },
      { name: "Mobile Sandbox", path: "/mobile-sandbox", icon: Smartphone },
      { name: "Driver Management", path: "/driver-management", icon: Car },
    ],
  },
  {
    title: "System & HR",
...
export function Sidebar() {
  ...
  return (
    <aside
      className={`w-64 bg-[#0a1628] border-r border-[#1a3a5c] flex flex-col h-screen shrink-0 ${GLOBAL_FONT}`}
    >
      <div className="p-6 border-b border-[#1a3a5c] shrink-0">
        <h1 className="!text-[20px] !font-black tracking-wider !text-[#f6b84b] !mb-0 uppercase">
          Britium Ops
        </h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar pb-24">
        {NAV_GROUPS.map((group) => (
          <div key={group.title}>
            <div className="text-[10px] font-black uppercase tracking-widest text-[#4d7a9b] mb-2 px-3">
              {group.title}
            </div>
              ...
              {group.links.map((link) => {
                ...
                return (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-200 text-[13px] font-semibold tracking-wide ${
                      isActive
      ...
      <div className="p-4 border-t border-[#1a3a5c] shrink-0 bg-[#0a1628]">
        <button
          ...
          className="w-full flex items-center gap-3 p-3 text-[#ff4f86] font-bold text-[13px] tracking-wide hover:bg-[#ff4f86]/10 rounded-xl transition-colors cursor-pointer"
        >
          <LogOut size={18} />
          <span>Sign Out</span>
        </button>
    ...
    <div className={`flex h-screen w-full bg-[#061524] overflow-hidden ${GLOBAL_FONT}`}>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-auto relative custom-scrollbar">
          <div className="min-w-[1200px] h-full p-4 md:p-6">{children}</div>
        </main>
      </div>
    </div>
  );
...
function LoginPageComponent() {
    ...
    try {
      ...
      alert(
        t(
          ...
          "အချက်အလက်များ မှားယွင်းနေပါသည်။ ပြန်လည်စစ်ဆေးပါ။",
        ),
      );
    } finally {
      setLoading(false);
  ...
  return (
    <div
      className={`flex min-h-screen items-center justify-center bg-[#061524] p-4 relative overflow-hidden notranslate ${GLOBAL_FONT}`}
      translate="no"
    >
      ...
      <button
        type="button"
        onClick={toggleLang}
        className="absolute top-6 right-6 z-20 bg-[#081b2e] border border-[#1a3a5c] text-[#eef8ff] px-4 py-2 rounded-xl text-[12px] font-bold tracking-wider cursor-pointer hover:border-[#f6b84b] transition-colors shadow-lg"
      >
        <span>{lang === "en" ? "မြန်မာဘာသာ" : "English"}</span>
      </button>
      ...
      <div className="w-full max-w-md p-10 bg-[#0b2236] border border-[#1a3a5c] rounded-3xl shadow-2xl z-10 relative">
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto bg-[#f6b84b] text-[#061524] rounded-2xl flex items-center justify-center text-3xl font-black mb-4 shadow-lg shadow-[#f6b84b]/20">
            BE
          </div>

          <h1 className="text-2xl font-black text-[#eef8ff] tracking-widest uppercase">
            <span>BRITIUM EXPRESS</span>
          </h1>

          <p className="text-[#f6b84b] text-[11px] font-bold tracking-[0.16em] uppercase mt-2">
            <span>
              {t("Enterprise Management Portal", "လုပ်ငန်းစီမံခန့်ခွဲမှု ဗဟိုစနစ်")}
            </span>
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          <div>
            <label className="block text-[12px] font-bold tracking-wider uppercase mb-2 text-[#4d7a9b]">
              <span>{t("Business Email", "လုပ်ငန်းသုံး အီးမေးလ်လိပ်စာ")}</span>
            </label>
            ...
            <input
              ...
              required
              className="w-full bg-[#081b2e] border border-[#1a3a5c] text-white p-4 rounded-xl outline-none focus:border-[#f6b84b] transition-colors text-[14px]"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-4 bg-[#f6b84b] text-[#061524] py-4 rounded-xl font-bold text-[14px] uppercase tracking-wider hover:bg-[#e5a93a] transition-colors disabled:opacity-50 cursor-pointer shadow-xl shadow-[#f6b84b]/10"
          >
      ...
      <Suspense fallback={<PageLoader />}>
        <Routes>
          ...
          <Route element={<AuthLayout />}>
            ...
            <Route path="/customer-service" element={<CustomerServiceCommandCenterPage />} />
            ...
            <Route path="/warehouse-ops" element={<WarehouseOperationPage />} />
            <Route path="/warehouse-operations" element={<Navigate to="/warehouse-ops" replace />} />
            ...
            <Route path="/business-development-manager" element={<BusinessDevelopmentManagerPage />} />
            <Route path="/marketing" element={<MarketingPage />} />
            <Route path="/marketing-portal" element={<MarketingPortalPage />} />
            <Route path="/tariff" element={<TariffPage />} />

            <Route path="/rider-management" element={<RiderPage />} />
            <Route path="/rider" element={<Navigate to="/rider-management" replace />} />

            <Route path="/mobile-sandbox" element={<RiderAppPage />} />
            <Route path="/rider-app" element={<Navigate to="/mobile-sandbox" replace />} />

            <Route path="/driver-management" element={<DriverPage />} />
            <Route path="/driver" element={<Navigate to="/driver-management" replace />} />
            ...
            <Route path="/delivery-dispatch" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/delivery-workflow" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/dispatch" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/dispatch-center" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/live-dispatch" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/exec-ops" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/ops-command" element={<Navigate to="/dispatch-command" replace />} />
            <Route path="/ops-manager" element={<Navigate to="/dispatch-command" replace />} />
          ...
          <Route path="/branch-office-process" element={<BranchOfficeProcessPage />} />
  <Route path="/branch-office-settlement" element={<BranchOfficeSettlementPage />} />
          <Route path="/waybill-print-studio" element={<WaybillPrintStudioPage />} />
                  <Route path="/invoice-print-studio" element={<InvoicePrintStudioPage />} />
        </Routes>
      </Suspense>
    </AppErrorBoundary>
  );
