...
export function resolvePostalCode(address: unknown, township: unknown): PostalMatch {
  ...
  // Bulk manifests frequently leave the Township column blank even though the
  // receiver address contains an explicit "... Township / မြို့နယ်" phrase.  The
  // previous guard discarded that stronger address evidence and left routing
  // UNRESOLVED.  rowsMentionedInAddress already requires a unique longest
  // township key, so it is safe to use for recovery even when the column is
  // blank.  Ambiguous matches still fall through as unresolved.
  const addressRows = rowsMentionedInAddress(addressKey);
