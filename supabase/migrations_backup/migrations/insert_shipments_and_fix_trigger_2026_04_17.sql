...
-- Create supply_chain_events table if referenced by a trigger
...
-- Get the admin user to use as merchant_id
DO $$
DECLARE
  v_merchant_id UUID;
  v_ygn_id UUID;
BEGIN
  ...
  -- Get Yangon warehouse
  SELECT id INTO v_ygn_id FROM public.warehouses WHERE code = 'YGN-HUB-01' LIMIT 1;
  
  IF v_merchant_id IS NULL THEN
    RAISE NOTICE 'No user profiles found, skipping shipment insert';
    RETURN;
  END IF;

  -- Insert shipments (ignore duplicates by awb)
  ...
  RAISE NOTICE 'Shipments inserted with merchant_id=%', v_merchant_id;
END $$;
