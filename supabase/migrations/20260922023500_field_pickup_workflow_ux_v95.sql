-- V95: Rider Pickup UX lifecycle metadata and safe action guidance.
...
begin
  ...
  from (
    select
      jsonb_strip_nulls(jsonb_build_object(
        ...
        'team_acceptance_status',coalesce(nullif(p.team_acceptance_status,''),'WAITING_TEAM_ACCEPTANCE'),
        ...
        'next_action',
          case
            ...
            when x.role_status<>'ACCEPTED' then 'ACCEPT_ASSIGNMENT'
            when upper(coalesce(p.team_acceptance_status,''))<>'TEAM_READY' then 'WAIT_FOR_TEAM_ACCEPTANCE'
  ...
  return jsonb_build_object(
    ...
    'source','be_portal_pickup_requests',
    'build','FIELD_PICKUP_REQUEST_OPTIONS_V95'
  );
end;
$$;
