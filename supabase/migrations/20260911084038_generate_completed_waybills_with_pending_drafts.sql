...
begin
  ...
  if nullif(v_pickup.pickup_proof_url, '') is null
     and not exists (
       select 1
  ...
  set
    ...
    photo_review_status = case when v_parcel_count = v_expected_count then 'APPROVED' else photo_review_status end,
...
CREATE OR REPLACE FUNCTION private.be_data_entry_waybill_sync_ready_v1(p_pickup_id text, p_sequences integer[], p_merchant_code text DEFAULT NULL::text, p_merchant_name text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  where pickup_id = v_pickup and parcel_sequence=any(p_sequences);
  ...
  where d.pickup_id = v_pickup and d.parcel_sequence=any(p_sequences)
  ...
  where pickup_id = v_pickup
    and waybill_no in (select delivery_way_id from public.be_data_entry_parcel_details where pickup_id=v_pickup and parcel_sequence=any(p_sequences))
...
$function$
;
revoke all on function private.be_data_entry_waybill_sync_ready_v1(text,integer[],text,text) from public,anon,authenticated;
...
begin
  ...
  v_response := jsonb_build_object(
    ...
    'canonical_pickup_preserved',true,'pending_parcels',v_expected-v_detail_count,'parcel_sequences',to_jsonb(v_sequences),'access',v_access
  );
