...
declare
  ...
  v_num integer;
begin
  if v_status not in ('DELIVERED','COMPLETED') then return new; end if;
  ...
  foreach v_role in array array['RIDER','DRIVER','HELPER'] loop
    v_code := case v_role when 'RIDER' then v_member.rider_code when 'DRIVER' then v_member.driver_code when 'HELPER' then v_member.helper_code end;
    v_name := case v_role when 'RIDER' then v_member.rider_name when 'DRIVER' then v_member.driver_name when 'HELPER' then v_member.helper_name end;

    v_email := null; v_num := null;
    if upper(coalesce(v_code,'')) ~ '^(RID|DRV|HLP)[0-9]+$' then
      v_num := regexp_replace(upper(v_code),'[^0-9]','','g')::integer;
      v_prefix := case v_role when 'RIDER' then 'rider_ygn_' when 'DRIVER' then 'driver_ygn_' when 'HELPER' then 'helper_ygn_' end;

      select r.email into v_email
      from public.be_user_account_registry r
      where lower(r.email) like v_prefix||'%@britiumventures.com'
        and nullif(regexp_replace(split_part(split_part(lower(r.email),'@',1),'_',3),'[^0-9]','','g'),'')::integer=v_num
        and coalesce(r.active,r.is_active,true)
      order by r.updated_at desc nulls last limit 1;

      if v_email is null then
        v_email := v_prefix||lpad(v_num::text,4,'0')||'@britiumventures.com';
      end if;
    end if;
    ...
    if v_email is not null then
      ...
      insert into public.be_commission_events(
        ...
        role_code,assignee_email,assignee_name,tracking_no,pickup_id,waybill_no,wayplan_code,
        rate_mmk,commission_mmk,event_status,actor_email,metadata
      ) values (
        'AUTO',new.delivery_way_id,v_work_date,'DELIVERY','PARCEL',1,
        v_role,v_email,v_name,new.delivery_way_id,new.pickup_id,new.waybill_no,new.wayplan_id,
        v_rate,v_rate,'READY',null,
        jsonb_build_object('source_adapter','WAYPLAN_DELIVERED_TRIGGER_V94','worker_code',v_code,'delivery_way_id',new.delivery_way_id,'wayplan_id',new.wayplan_id)
