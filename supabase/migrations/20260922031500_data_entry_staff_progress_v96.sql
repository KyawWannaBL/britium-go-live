-- V96: Data Entry multi-staff live progress by merchant/pickup/date.

create or replace function public.be_data_entry_staff_progress_v96(
...
as $$
declare
  v_date date:=coalesce(p_pickup_date,(now() at time zone 'Asia/Yangon')::date);
...
begin
  ...
  with relevant as (
    ...
    where p.pickup_date=v_date
      and nullif(btrim(coalesce(p.pickup_id,'')),'') is not null
  ...
  drafts as (
    select
      d.pickup_id,
      lower(coalesce(nullif(btrim(p.email),''),u.email::text,'UNKNOWN')) email,
    ...
    group by d.pickup_id,lower(coalesce(nullif(btrim(p.email),''),u.email::text,'UNKNOWN'))
  ),
  saved_totals as (
    select pickup_id,count(distinct parcel_sequence)::integer registered_count
    from public.be_data_entry_parcel_details d
    where exists(select 1 from relevant r where r.pickup_id=d.pickup_id)
    group by pickup_id
  ),
  draft_totals as (
    select pickup_id,count(distinct parcel_sequence)::integer draft_count
    from public.be_data_entry_pending_drafts d
    where exists(select 1 from relevant r where r.pickup_id=d.pickup_id)
    group by pickup_id
  ),
    ...
    select
      ...
      coalesce((
        select jsonb_agg(jsonb_build_object(
          'email',ce.email,
          'name',coalesce(nullif(pr.full_name,''),nullif(pr.email,''),ce.email),
  ...
  saved as (
    select d.pickup_id,lower(coalesce(nullif(btrim(d.saved_by_email),''),'UNKNOWN')) email,
           count(distinct d.parcel_sequence)::integer saved_rows,max(coalesce(d.saved_at,d.updated_at,d.created_at)) last_saved_at
    from public.be_data_entry_parcel_details d join relevant r on r.pickup_id=d.pickup_id
    group by d.pickup_id,lower(coalesce(nullif(btrim(d.saved_by_email),''),'UNKNOWN'))
  ),
  drafts as (
    select d.pickup_id,lower(coalesce(nullif(btrim(p.email),''),u.email::text,'UNKNOWN')) email,
           count(distinct d.parcel_sequence)::integer draft_rows,max(d.updated_at) last_draft_at
    from public.be_data_entry_pending_drafts d join relevant r on r.pickup_id=d.pickup_id
    left join public.profiles p on p.id=d.owner_id
    left join auth.users u on u.id=d.owner_id
    group by d.pickup_id,lower(coalesce(nullif(btrim(p.email),''),u.email::text,'UNKNOWN'))
  ),
  observed as (
    select email from saved union select email from drafts
  ),
  active_staff as (
    select lower(p.email) email,p.full_name,p.id
    from public.profiles p
    where coalesce(p.is_active,true)
      and coalesce(p.is_approved,true)
      and lower(coalesce(p.status,'active')) not in ('disabled','inactive','suspended')
      and replace(replace(lower(coalesce(p.role,'')),'_','-'),' ','-')='data-entry'
      and lower(coalesce(p.email,'')) not like 'test%'
      and lower(coalesce(p.email,'')) not like 'uat_probe_%'
    union
    select o.email,coalesce(p.full_name,o.email),p.id
    from observed o left join public.profiles p on lower(p.email)=o.email
    where o.email<>'UNKNOWN'
  ),
  staff_rows as (
    select
      a.email,
      coalesce(nullif(max(a.full_name),''),a.email) display_name,
      ...
      count(distinct r.merchant_code) filter (where coalesce(s.saved_rows,0)>0 or coalesce(d.draft_rows,0)>0)::integer merchants_touched,
      greatest(max(s.last_saved_at),max(d.last_draft_at)) last_activity
    from active_staff a
    left join saved s on s.email=a.email
    left join drafts d on d.email=a.email and (s.pickup_id is null or d.pickup_id=s.pickup_id)
    left join relevant r on r.pickup_id=coalesce(s.pickup_id,d.pickup_id)
    group by a.email
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    'email',email,
  ...
  ) order by (email=v_email) desc,saved_rows desc,draft_rows desc,display_name),'[]'::jsonb)
  into v_staff
  from staff_rows;
  ...
  merchant_groups as (
    select
      ...
      jsonb_agg(jsonb_build_object(
        ...
        'status',item->>'status',
        'registered',(item->>'registered')::integer,
        'expected',(item->>'expected')::integer,
        'contributors',item->'contributors'
      ) order by item->>'pickup_id') pickup_details
    from pickup_json
  ...
  return jsonb_build_object(
    ...
    'staff',v_staff,
    'generated_at',now(),
    'build','DATA_ENTRY_STAFF_PROGRESS_V96'
  );
end;
$$;

revoke all on function public.be_data_entry_staff_progress_v96(date) from public,anon;
grant execute on function public.be_data_entry_staff_progress_v96(date) to authenticated;
