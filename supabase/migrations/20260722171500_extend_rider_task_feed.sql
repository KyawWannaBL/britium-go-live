-- Extend the authenticated Rider task feed with delivery-recipient fields.
    ...
    jsonb_agg(
      jsonb_strip_nulls(
        jsonb_build_object(
          ...
          'delivery_way_id', nullif(
            x.record ->> 'delivery_way_id',
            ''
          ),
          'receiver_name', coalesce(
            nullif(x.record ->> 'recipient_name', ''),
            nullif(x.record ->> 'receiver_name', '')
          ),
          'receiver_phone', coalesce(
            nullif(x.record ->> 'recipient_phone', ''),
            nullif(x.record ->> 'receiver_phone', '')
          ),
          'receiver_address', coalesce(
            nullif(x.record ->> 'delivery_address', ''),
            nullif(x.record ->> 'receiver_address', '')
          ),
          'receiver_township', coalesce(
            nullif(x.record ->> 'delivery_township', ''),
            nullif(x.record ->> 'receiver_township', '')
          ),
          'receiver_city', coalesce(
            nullif(x.record ->> 'delivery_city', ''),
            nullif(x.record ->> 'city_region', '')
          ),
          'service_type', coalesce(
            nullif(x.record ->> 'service_type', ''),
            nullif(x.record ->> 'service_tier', ''),
            'Standard'
          ),
          'notes', coalesce(
            nullif(x.record ->> 'special_instructions', ''),
            nullif(x.record ->> 'assignment_note', ''),
            nullif(x.record ->> 'remarks', '')
          ),
          'task_type', case
            when upper(coalesce(
              nullif(x.record ->> 'delivery_status', ''),
              nullif(x.record ->> 'dispatch_status', ''),
              ''
            )) in (
              'ASSIGNED',
              'ASSIGNED_TO_RIDER',
