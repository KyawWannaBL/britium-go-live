...
begin
  ...
  select s.wayplan_id,
    ...
    into v_wayplan_id,v_parent_pickup_id,v_stop_status,v_wayplan_status,v_rider_code,v_driver_code,v_helper_code,v_required_cod,v_has_scan
  ...
  if v_action in ('start_delivery','out_for_delivery') then
    ...
    update public.be_wayplan_dispatch_stops
       set stop_status='OUT_FOR_DELIVERY',rider_status='OUT_FOR_DELIVERY',dispatch_status='OUT_FOR_DELIVERY',updated_at=now(),
           metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('rider_mobile_action','START_DELIVERY','worker_code',v_code,'worker_role',v_role,'action_at',now())
    ...
    update public.be_waybill_ledger
       set dispatch_status='OUT_FOR_DELIVERY',wayplan_status='OUT_FOR_DELIVERY',rider_status='OUT_FOR_DELIVERY',updated_at=now(),
           metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('rider_mobile_action','START_DELIVERY','worker_code',v_code,'action_at',now())
  ...
  if v_action in ('deliver','delivered','verify_delivery','delivery_verified') then
    ...
    update public.be_wayplan_dispatch_stops
       set stop_status='DELIVERED',rider_status='DELIVERED',dispatch_status='DELIVERED',delivered_at=coalesce(delivered_at,now()),
           ...
           metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('rider_mobile_action','DELIVERED','worker_code',v_code,'worker_role',v_role,'required_cod',v_required_cod,'cod_collected',v_collected_cod,'proof_url',v_proof_url,'action_at',now())
     where delivery_way_id=v_delivery_way_id and wayplan_id=v_wayplan_id;
    ...
    update public.be_waybill_ledger
       set dispatch_status='DELIVERED',wayplan_status='DELIVERED',rider_status='DELIVERED',delivered_at=coalesce(delivered_at,now()),cod_collected=v_collected_cod,rider_proof_url=v_proof_url,failed_reason=null,updated_at=now(),
           metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('rider_mobile_action','DELIVERED','worker_code',v_code,'required_cod',v_required_cod,'cod_collected',v_collected_cod,'proof_url',v_proof_url,'action_at',now())
     where delivery_way_id=v_delivery_way_id or tracking_no=v_delivery_way_id;
    ...
    return jsonb_build_object('ok',true,'delivery_way_id',v_delivery_way_id,'pickup_id',v_parent_pickup_id,'wayplan_id',v_wayplan_id,'action','deliver','mobile_status','DELIVERED','required_cod',v_required_cod,'cod_collected',v_collected_cod,'proof_url',v_proof_url);
  end if;
  ...
  if v_action='exception' or v_action like '%delivery_exception%' or v_action like '%delivery_failed%' then
    ...
    update public.be_wayplan_dispatch_stops
       set stop_status=case when v_failure_status='RTO' then 'RETURN_TO_WAREHOUSE' else 'FAILED_DELIVERY' end,
           ...
           metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('rider_mobile_action','DELIVERY_EXCEPTION','worker_code',v_code,'worker_role',v_role,'failure_status',v_failure_status,'reason',v_reason,'proof_url',v_proof_url,'action_at',now())
     where delivery_way_id=v_delivery_way_id and wayplan_id=v_wayplan_id;
    ...
    update public.be_waybill_ledger
       set dispatch_status=v_failure_status,rider_status=v_failure_status,failed_reason=v_reason,rider_proof_url=coalesce(v_proof_url,rider_proof_url),updated_at=now(),
           metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('rider_mobile_action','DELIVERY_EXCEPTION','worker_code',v_code,'failure_status',v_failure_status,'reason',v_reason,'proof_url',v_proof_url,'action_at',now())
     where delivery_way_id=v_delivery_way_id or tracking_no=v_delivery_way_id;
    ...
    return v_failure||jsonb_build_object('ok',true,'delivery_way_id',v_delivery_way_id,'pickup_id',v_parent_pickup_id,'wayplan_id',v_wayplan_id,'action','exception','mobile_status',case when v_failure_status='RTO' then 'RTO' else 'DELIVERY_FAILED' end,'proof_url',v_proof_url);
  end if;
...
declare
  ...
  v_action text:=lower(btrim(coalesce(p_payload->>'action',p_payload->>'source_action','')));
  v_id text:=upper(btrim(coalesce(p_payload->>'delivery_way_id',p_payload->>'pickup_id',p_payload->>'pickup_way_id','')));
  v_process text:=lower(btrim(coalesce(p_payload->>'process_type',p_payload->>'workflow_area','')));
begin
  if auth.uid() is null then return jsonb_build_object('ok',false,'error','AUTHENTICATED_FIELD_SESSION_REQUIRED'); end if;
  if v_role not in ('rider','driver','helper') then return jsonb_build_object('ok',false,'error','FIELD_ROLE_NOT_RECOGNIZED'); end if;
  if v_id ~ '^D[0-9]{4}-[A-Z0-9]+-[0-9]{3}$' or v_action in ('start_delivery','out_for_delivery','deliver','delivered','verify_delivery','delivery_verified') or (v_action='exception' and v_process='delivery') or v_action like '%delivery_exception%' or v_action like '%delivery_failed%' then
    return public.be_field_team_delivery_action(p_payload);
  end if;
  if v_role='helper' and v_action in ('verify_pickup','pickup_verify','pickup_verified','verify','collect','pickup_collected','collected','delivered_to_warehouse') then
    return jsonb_build_object('ok',false,'error','PRIMARY_WORKER_REQUIRED','message','Only assigned rider or driver can finalize pickup or delivery. Helper may upload evidence and report exceptions.');
  end if;
...
$$;
revoke all on function public.be_rider_pickup_action(jsonb) from public;
revoke all on function public.be_rider_pickup_action(jsonb) from anon;
