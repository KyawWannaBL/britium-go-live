-- V136: two-step non-blocking location defaults.
-- If an exact provider location is unavailable, use the accepted 7-digit
-- postal/ward default first, then the accepted township default. Both are
-- deliberately generic routing points and must not create LOCATION_PENDING.
...
begin
  ...
  with base as (
    select
      ...
      (
        ...
        and (
          loc.match_level in ('ADDRESS_EXACT','POI_EXACT','POSTAL_DEFAULT','TOWNSHIP_DEFAULT')
          or (
  ...
  return jsonb_build_object(
    ...
    'build','WAYPLAN_VISIBLE_QUEUE_V136_GENERIC_DEFAULTS_20260924'
  );
end
$function$;
...
is 'V136: exact provider pins plus accepted 7-digit postal/ward and township generic defaults are route-ready; location review is not required for recognized defaultable destinations.';

            and loc.match_level in ('ADDRESS_EXACT','POI_EXACT')
          )
          or (
            upper(coalesce(loc.coordinate_source,'')) = 'POSTAL_WARD_DEFAULT_V136'
            and loc.match_level = 'POSTAL_DEFAULT'
          )
          or (
            upper(coalesce(loc.coordinate_source,'')) = 'TOWNSHIP_DEFAULT_V136'
            and loc.match_level = 'TOWNSHIP_DEFAULT'
          )
        )
        and loc.coordinate_source<>'QUARANTINED_BULK_COORDINATE_V30'
        and not (round(loc.latitude::numeric,5)=16.80000 and round(loc.longitude::numeric,5)=96.15000)
...
comment on function public.be_wayplan_visible_queue_v124(text,integer)
is 'V136: exact validated Google/Mapbox/manual locations are route-ready; approximate Mapbox results remain LOCATION_PENDING.';
