-- V121: pickup-scoped Waybill Studio snapshot.
-- Avoid losing rows when a selected pickup spans outside the global newest-500 window.

create or replace function public.be_waybill_studio_snapshot_pickup_v121(
  p_pickup_id text,
...
begin
  ...
  select coalesce(jsonb_agg(x.row_json order by x.created_at desc, x.waybill_no), '[]'::jsonb)
    into v_rows
  from (
    select
      p.created_at,
      ...
      case when d.id is null then '{}'::jsonb else
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'waybill_data_source', 'DATA_ENTRY_PICKUP_SCOPED_V121'
        ))
      end as row_json
    ...
    where p.pickup_id=v_pickup
      and nullif(btrim(p.waybill_no), '') is not null
    order by p.created_at desc, p.waybill_no
  ...
  return jsonb_build_object(
    ...
    'source','DATA_ENTRY_PICKUP_SCOPED_V121'
  );
end
$function$;

grant execute on function public.be_waybill_studio_snapshot_pickup_v121(text,integer) to authenticated;
