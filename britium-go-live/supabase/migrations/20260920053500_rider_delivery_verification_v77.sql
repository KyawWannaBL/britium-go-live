...
-- Mobile snapshot compatibility: keep internal consolidated-bulk row IDs for
-- backend integrity while showing the original operational D... Way ID.
...
begin
  ...
  from (
    select
      jsonb_strip_nulls(jsonb_build_object(
        ...
        'pickup_status',st.mobile_status,
        'workflow_stage',st.mobile_status,
        'rider_app_stage',st.mobile_status,
        ...
        'warehouse_status','WAREHOUSE_ACCEPTED',
        ...
        'rider_proof_url',coalesce(s.rider_proof_url,s.proof_url,wl.rider_proof_url),
        'proof_url',coalesce(s.rider_proof_url,s.proof_url,wl.rider_proof_url),
        'delivered_at',coalesce(s.delivered_at,wl.delivered_at),
        'failed_reason',coalesce(s.failed_reason,wl.failed_reason),
    ...
    left join lateral(
      select ll.*
      from public.be_waybill_ledger ll
      where ll.delivery_way_id=s.delivery_way_id or ll.tracking_no=s.delivery_way_id
  ...
  select coalesce(jsonb_agg(x.job order by x.sort_at desc nulls last),'[]'::jsonb)
  into v_jobs
  from (
    select e job,
           nullif(e->>'updated_at','')::timestamptz sort_at
  ...
  return v_base
    || jsonb_build_object(
      ...
      'build','RIDER_MOBILE_OPERATIONAL_WAY_ID_V77',
      'counts',jsonb_build_object(
        'jobs',jsonb_array_length(v_jobs),
        'delivery_jobs',(
          select count(*)
          from jsonb_array_elements(v_jobs) e
          where coalesce(e->>'job_kind','PICKUP')='DELIVERY'
        ),
        ...
        'unread',(
          select count(*)
          from jsonb_array_elements(coalesce(v_base->'notifications','[]'::jsonb)) e
          where coalesce((e->>'is_read')::boolean,false)=false
        )
      )
    );
end;
...
-- Final compact V77 mobile snapshot implementation (PostgreSQL functions accept
-- at most 100 arguments; keep each jsonb_build_object below that limit).
create or replace function public.be_field_team_mobile_snapshot_v77(p_payload jsonb default '{}'::jsonb)
returns jsonb
language plpgsql
security definer
