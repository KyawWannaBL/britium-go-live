...
-- Britium Waybill v12.5
-- Data Entry is authoritative for printable parcel fields.
-- City / region remain available as metadata but are never used as a township fallback by the UI.
...
begin
  ...
  from (
    select
      ...
      case when d.id is null then '{}'::jsonb else
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'waybill_data_source', 'DATA_ENTRY_AUTHORITATIVE_V12_5'
        ))
      end as row_json
    ...
    left join lateral (
      ...
      where
        upper(nullif(btrim(d0.delivery_way_id), '')) = upper(nullif(btrim(p.waybill_no), ''))
        or (
  ...
  return jsonb_build_object(
    ...
    'source', 'DATA_ENTRY_AUTHORITATIVE_V12_5'
  );
end;
$function$;

-- Preserve existing callers: old v12.2 clients now receive the same live Data Entry-authoritative snapshot.
...
as $function$
begin
  return public.be_waybill_studio_snapshot_v12_5(p_limit);
end;
$function$;
