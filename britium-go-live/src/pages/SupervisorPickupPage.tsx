...
export default function SupervisorPickupPage() {
  ...
  // 1) Rider pickup: Rider only (bicycle / rider workflow).
  // 2) Vehicle pickup: Driver + Helper + Fleet Vehicle. Rider can be NIL.
  const isVehicleDispatch = Boolean(selectedDriver);
  const canConfirmAssignment = Boolean(
    selectedPickup &&
      (isVehicleDispatch
        ? selectedDriver && selectedHelper && selectedFleet
        : selectedRider),
  );
  ...
  async function confirmAssignment() {
    ...
    // Vehicle dispatch: Driver + Helper + Fleet are required. Rider is optional / NIL.
    if (selectedDriver) {
      if (!selectedHelper) {
        setMessage("Please select a Helper for the driver pickup.");
        return;
      }
      if (!selectedFleet) {
        setMessage("Please select a Fleet Vehicle for the driver pickup.");
        return;
      }
    }

    // Rider dispatch: when no driver is selected, a Rider is required.
    if (!selectedDriver && !selectedRider) {
      setMessage("Please select either a Rider or a Driver.");
      return;
    }
    ...
    // Resolve actual workforce codes and vehicle plate from dropdown selections.
    const riderCode = selectedRider
      ? (riders.find((r) => workerValue(r) === selectedRider)?.code || selectedRider)
      : null;
    const driverCode = selectedDriver
      ? (drivers.find((d) => workerValue(d) === selectedDriver)?.code || selectedDriver)
      : null;
    const helperCode = selectedHelper
      ? (helpers.find((h) => workerValue(h) === selectedHelper)?.code || selectedHelper)
      : null;
    const vehiclePlate = selectedFleet
      ? (fleets.find((f) => f.id === selectedFleet)?.vehicle_no || selectedFleet)
      : null;

    try {
      toast.loading(`Assigning ${selectedPickup.pickup_id}...`, { id: "assign-toast" });

      const { data, error } = await (supabase as any).rpc("be_assign_pickup_field_team", {
        p_pickup_id: selectedPickup.pickup_id,
        p_rider_code: riderCode,
        p_driver_code: driverCode,
        p_helper_code: helperCode,
  ...
  const errorCount = selectedPickup && !canConfirmAssignment ? 1 : 0;
  const warningCount =
    (!riders.length ? 1 : 0) +
    (!drivers.length ? 1 : 0) +
    (!helpers.length ? 1 : 0) +
        ...
        <div className="space-y-6 min-w-0">
          <div className="bg-[#0b2236] border border-[#1a3a5c] rounded-2xl flex flex-col p-6 min-w-0">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div><label className="block text-[#4d7a9b] text-[11px] uppercase tracking-widest mb-2">Rider - optional if Driver selected</label><select value={selectedRider} onChange={(e) => setSelectedRider(e.target.value)} className="w-full bg-[#061524] border border-[#1a3a5c] text-[#eef8ff] p-3 rounded-xl outline-none focus:border-[#f6b84b] text-[13px] cursor-pointer"><option value="">NIL / No Rider</option>{eligibleRiders.map((rider) => <option key={workerValue(rider)} value={workerValue(rider)}>{rider.code} - {rider.name}{rider.email ? ` (${rider.email})` : ""}</option>)}</select></div>
          ...
          <div className="bg-[#0b2236] border border-[#1a3a5c] p-6 rounded-2xl">
            ...
            <div className="space-y-3">{[
              ["Supervisor queue source", "be_v_supervisor_pickup_queue", "OK"],
              ["Pickup request selected", selectedPickup?.pickup_id || "Select a pending pickup request.", selectedPickup?.pickup_id ? "OK" : "WARN"],
              [
                "Dispatch mode",
                selectedDriver ? "Vehicle pickup: Driver + Helper + Fleet required. Rider may be NIL." : selectedRider ? "Rider pickup: Rider assigned." : "Select a Rider, or select a Driver for vehicle pickup.",
                canConfirmAssignment ? "OK" : selectedPickup ? "ERROR" : "WARN",
              ],
              [
                "Rider assignment",
                selectedRider || (selectedDriver ? "NIL / not required for vehicle pickup." : riders.length ? "Select an active rider." : "No active rider loaded."),
                selectedRider || selectedDriver ? "OK" : riders.length ? "WARN" : "ERROR",
              ],
              [
                "Driver assignment",
                selectedDriver || (drivers.length ? "Optional for rider pickup." : "No active driver loaded."),
                selectedDriver || selectedRider ? "OK" : drivers.length ? "WARN" : "ERROR",
              ],
              [
                "Helper assignment",
                selectedHelper || (selectedDriver ? "Required for vehicle pickup." : "Not required for rider pickup."),
                selectedDriver ? (selectedHelper ? "OK" : "ERROR") : "OK",
              ],
              [
                "Fleet vehicle",
                selectedFleet || (selectedDriver ? "Required for vehicle pickup." : "Not required for rider pickup."),
                selectedDriver ? (selectedFleet ? "OK" : "ERROR") : "OK",
              ],
              ["Supervisor note length", `${supervisorNote.length}/500`, supervisorNote.length <= 500 ? "OK" : "ERROR"],
            ].map(([label, detail, status]) => <div key={label} className="bg-[#061524] border border-[#1a3a5c] p-3 rounded-xl min-w-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[#eef8ff] text-[12px] uppercase tracking-wider break-words">{label}</div><div className="text-[#4d7a9b] text-[12px] mt-1 break-words">{detail}</div></div><span className={`text-[10px] font-black px-2 py-1 rounded-full border shrink-0 ${status === "OK" ? "text-[#22c55e] border-[#0d6b4c] bg-[#08251b]" : status === "ERROR" ? "text-[#ff4f93] border-[#ff4f93]/40 bg-[#2a1934]" : "text-[#f6b84b] border-[#f6b84b]/40 bg-[#30250b]"}`}>{status}</span></div></div>)}</div>
          </div>
        </div>
      </div>
    </div>
