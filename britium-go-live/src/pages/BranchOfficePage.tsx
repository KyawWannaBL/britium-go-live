...
// ─── SAFE VALUE / BACKEND NORMALIZATION ──────────────────────────────────────
...
function normalizeLooseRow(value: any): Record<string, any> {
  const flat = flattenPayload(value);
  const normalized: Record<string, any> = {};

  Object.entries(flat).forEach(([key, item]) => {
    normalized[key] =
...
function normalizeOfficeRow(value: any): Record<string, any> {
  const row = normalizeLooseRow(value);
  return {
    ...row,
    branch_code: safeText(row.branch_code ?? row.code, ""),
    branch_name: safeText(row.branch_name ?? row.display_name ?? row.name, ""),
    city: safeText(row.city, ""),
    region: safeText(row.region ?? row.state_region, ""),
    address: safeText(row.address, ""),
    phone: safeText(row.phone ?? row.phone_primary, ""),
    manager_name: safeText(row.manager_name ?? row.branch_manager, ""),
    manager_phone: safeText(row.manager_phone, ""),
  };
}

function normalizeShipmentRow(value: any): Record<string, any> {
  const row = normalizeLooseRow(value);
  return {
    ...row,
    id: row.id,
    branch_code: safeText(row.branch_code, ""),
    tracking_no: safeText(row.tracking_no ?? row.waybill_id, ""),
    merchant: safeText(row.merchant ?? row.merchant_name ?? row.merchant_code, ""),
    receiver: safeText(row.receiver ?? row.receiver_name ?? row.customer_name, ""),
    township: safeText(row.township ?? row.destination_township, ""),
    address: safeText(row.address ?? row.delivery_address, ""),
    phone: safeText(row.phone ?? row.receiver_phone, ""),
    status: safeText(row.status ?? row.record_status, "pending").toLowerCase(),
    weight_kg: safeNumber(row.weight_kg),
    cod_amount: safeNumber(row.cod_amount),
    delivery_fee: safeNumber(row.delivery_fee),
    item_value: safeNumber(row.item_value),
    finance_locked: safeNumber(row.finance_locked),
    created_at: safeText(row.created_at, ""),
  };
}

function normalizeSnapshot(value: any): Record<string, number> {
  ...
  return {
    total_shipments: safeNumber(raw.total_shipments),
    in_transit: safeNumber(raw.in_transit),
    delivered: safeNumber(raw.delivered),
    pending: safeNumber(raw.pending),
    cod_total: safeNumber(raw.cod_total),
    total_staff: safeNumber(raw.total_staff),
    pending_amendments: safeNumber(raw.pending_amendments),
  };
}
...
const getBranchOffices = async () => {
  ...
  return rowsFrom(data).map(normalizeOfficeRow);
};

const getBranchSnapshot = async (branchCode: string) => {
  const { data, error } = await supabase.rpc("be_get_branch_snapshot", {
...
const getBranchShipments = async (branchCode: string) => {
  const { data, error } = await supabase
    .from("branch_shipments")
    .select("*")
    .eq("branch_code", branchCode)
    .order("created_at", { ascending: false });
  if (error) throw error;
  return rowsFrom(data).map(normalizeShipmentRow);
};

const createBranchShipment = async (payload: any) => {
  const { data, error } = await supabase
    .from("branch_shipments")
    .insert([{ ...payload, status: "pending", finance_locked: 0 }])
    .select();
  if (error) throw error;
  return data;
};

const updateBranchShipment = async (id: number, payload: any, mode: string) => {
  const { error } = await supabase.from("branch_shipments").update(payload).eq("id", id);
  if (error) return { ok: false, message: error.message };
  return { ok: true };
};

const lockBranchShipment = async (id: number) => {
  const { error } = await supabase
    .from("branch_shipments")
    .update({ finance_locked: 1 })
    .eq("id", id);
  if (error) throw error;
};

const unlockBranchShipment = async (id: number, mode: string) => {
  if (mode !== "hq") throw new Error("Unauthorized");
  const { error } = await supabase
    .from("branch_shipments")
    .update({ finance_locked: 0 })
    .eq("id", id);
  if (error) throw error;
};

const getBranchStaff = async (branchCode: string) => {
  const { data, error } = await supabase
    .from("branch_staff")
    .select("*")
    .eq("branch_code", branchCode);
  if (error) throw error;
  return rowsFrom(data).map(normalizeLooseRow);
};

const addBranchStaff = async (payload: any, mode: string) => {
  const { error } = await supabase.from("branch_staff").insert([payload]);
  if (error) throw error;
};

const updateBranchStaff = async (id: number, payload: any, mode: string) => {
  const { error } = await supabase.from("branch_staff").update(payload).eq("id", id);
  if (error) throw error;
};
...
const getBranchAmendments = async (branchCode: string) => {
  const { data, error } = await supabase
...
const getAllPendingAmendments = async () => {
  const { data, error } = await supabase
    .from("amendment_requests")
    .select("*")
    .eq("status", "pending")
...
const submitAmendmentRequest = async (payload: any) => {
  const { error } = await supabase
    .from("amendment_requests")
    .insert([{ ...payload, status: "pending", request_no: `AM-${Date.now()}` }]);
  if (error) throw error;
};
...
const reviewAmendment = async (
  ...
  const { error } = await supabase
    .from("amendment_requests")
    .update({ status: action, reviewed_by: reviewer, review_note: note })
    .eq("id", id);
  if (error) throw error;
};
...
const calculateFee = async (
  ...
  const { data, error } = await supabase.rpc("be_calculate_tariff", {
    p_origin: origin,
    p_dest: dest,
    p_weight: weight,
  });
  ...
  const row = Array.isArray(data) ? data[0] || {} : isObject(data) ? data : {};
  return {
    base_fee: safeNumber(row.base_fee),
    default_weight: safeNumber(row.default_weight, 3),
    weight_fee: safeNumber(row.weight_fee),
    total_fee: safeNumber(row.total_fee),
  };
};
...
const updateTariff = async (id: number, payload: any) => {
  const { error } = await supabase.from("tariffs").update(payload).eq("id", id);
  if (error) throw error;
};

const fetchServiceAreas = async (branchCode?: string) => {
...
const addServiceArea = async (payload: any) => {
...
function fmt(value: unknown) { return safeNumber(value).toLocaleString(); }

function statusBadge(value: unknown) {
  const s = safeText(value, "unknown").toLowerCase();
...
const btn = (v: string) => ({
  gold:    "bg-[#C09B30] text-[#0A1628] font-semibold px-4 py-2.5 rounded-xl text-[13px] hover:bg-[#f6b84b] transition-colors cursor-pointer disabled:opacity-50",
  ghost:   "bg-[#0b2236] border border-[#1a3a5c] text-[#eef8ff] px-4 py-2.5 rounded-xl text-[13px] hover:border-[#f6b84b] transition-colors cursor-pointer disabled:opacity-50",
  danger:  "bg-rose-900/30 border border-rose-800 text-rose-300 px-4 py-2.5 rounded-xl text-[13px] hover:bg-rose-900/60 transition-colors cursor-pointer disabled:opacity-50",
  emerald: "bg-emerald-900/30 border border-emerald-700 text-emerald-300 px-4 py-2.5 rounded-xl text-[13px] hover:bg-emerald-800/60 transition-colors cursor-pointer disabled:opacity-50",
  sm:      "bg-[#0b2236] border border-[#1a3a5c] text-[#eef8ff] px-3 py-1.5 rounded-lg text-[12px] hover:border-[#f6b84b] transition-colors cursor-pointer disabled:opacity-50",
}[v]);
...
function OverviewTab({ snap, office }: { snap: any | null; office: any | null }) {
    ...
    <div className="space-y-6">
      <div className="bg-[#061524] border border-[#1a3a5c] rounded-2xl p-5 space-y-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[#f6b84b] text-[13px] font-semibold uppercase tracking-widest">{safeText(office.branch_name, "Branch Office")}</span>
        </div>
        {[
          ...
          <div key={k} className="flex gap-3 text-[13px]">
            ...
            <span className="text-[#eef8ff]">{safeText(v)}</span>
          </div>
        ))}
      </div>
...
function DataEntryTab({ branchCode, mode }: { branchCode: string; mode: "hq" | "branch" }) {
  ...
  const [feeBreakdown, setFeeBreakdown] = useState<any | null>(null);
...
function ShipmentsTab({ branchCode, mode }: { branchCode: string; mode: "hq" | "branch" }) {
  ...
  const [amendTarget, setAmendTarget] = useState<{ shipment: any; field: FinanceFieldName } | null>(null);
...
function WayplanTab({ branchCode }: { branchCode: string }) {
  ...
  // Group by township
  const byTownship: Record<string, any[]> = {};
  pending.forEach(s => {
  ...
  function printRoute(township: string, items: any[]) {
...
function TeamTab({ branchCode, mode }: { branchCode: string; mode: "hq" | "branch" }) {
  ...
  async function handleStatusToggle(s: any) {
    try {
              ...
              <div key={r.id} className="bg-[#0b2236] border border-[#1a3a5c] rounded-2xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-[12px]">
                  {[["Branch",r.branch_code],["Tracking No",r.tracking_no],["Field",FINANCE_FIELD_LABELS[r.field_name as FinanceFieldName]??r.field_name],["Current",r.current_value],["Requested",r.requested_value]].map(([k,v]) => (
                    <div key={k}><span className="text-[#4d7a9b]">{k}: </span><span className="text-[#eef8ff]">{safeText(v)}</span></div>
                  ))}
                </div>
          ...
          <div className="space-y-4">
            <div className="bg-[#061524] border border-[#1a3a5c] rounded-xl p-4 space-y-2 text-[13px]">
              {[["Branch",reviewTarget.branch_code],["Tracking",reviewTarget.tracking_no],["Field",FINANCE_FIELD_LABELS[reviewTarget.field_name as FinanceFieldName]??reviewTarget.field_name],["Current Value",reviewTarget.current_value],["Requested Value",reviewTarget.requested_value],["Reason",reviewTarget.reason]].map(([k,v]) => (
                <div key={k} className="flex gap-2"><span className="text-[#4d7a9b] w-36 shrink-0">{k}</span><span className="text-[#eef8ff]">{safeText(v)}</span></div>
              ))}
            </div>
...
export default function BranchPortalPage() {
  const [branchCode, setBranchCode] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem("branch_code") ?? "YGN";
    }
    return "YGN";
  });
  const [mode, setMode] = useState<"hq" | "branch">(() => {
    if (typeof window !== 'undefined') {
      return (sessionStorage.getItem("branch_mode") as "hq" | "branch") ?? "branch";
    }
    return "branch";
  });
  ...
  const currentOffice =
    offices.find((office) => safeText(office.branch_code, "") === branchCode) ?? {
      branch_code: branchCode,
      branch_name: BRANCHES.find((branch) => branch.code === branchCode)?.label ?? branchCode,
      city: branchCode === "YGN" ? "Yangon" : branchCode === "MDY" ? "Mandalay" : "Naypyitaw",
      region: "",
      address: "",
      phone: "",
      manager_name: "",
