...
export default function CustomerPortalPage() {
  ...
  function search() {
    ...
    if (!trackingNo || !phoneNo) {
      setMessage("Enter both the Way ID / Waybill and the recipient phone number.");
      return;
    }
    setMessage("");
    setQuery({ p_tracking_no: trackingNo, p_phone: phoneNo, p_actor_email: null });
  }
  ...
  return (
    <div>
      <div className="p-4 bg-[#0b2236] border border-[#1a3a5c] rounded-2xl mx-4 mt-4">
        ...
        <div className="text-[#8ab0c9] text-sm mt-1">Enter both the Way ID / Waybill and recipient phone number.</div>
        <div className="flex gap-2 mt-3">
          ...
          <button onClick={search} className="bg-[#35bdf4] text-[#061524] px-4 py-2 rounded-lg font-semibold">Search</button>
        </div>
        {message && <div className="mt-2 text-sm text-[#ff8ca8]">{message}</div>}
      </div>
