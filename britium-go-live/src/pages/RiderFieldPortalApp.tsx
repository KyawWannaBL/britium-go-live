...
async function fetchRiderPayload(login: string) {
  ...
  try {
    ...
    if (!error && data?.ok !== false) {
      ...
      return {
        ...
        jobs,
        notifications,
        source: `${data?.source || "rpc:be_field_team_mobile_snapshot"} / ${normalizedLogin}`,
      };
    }
  } catch (err) {
  ...
  if (role === "rider") {
    try {
      ...
      if (rows.length > 0) {
        ...
        return {
          ...
          jobs: rows.map((row: any) => ({ ...row, mobile_role: role })),
    ...
    if (!error && data) {
      return {
        ...
        jobs: Array.isArray(data.jobs) ? data.jobs.map((row: any) => ({ ...row, mobile_role: role })) : [],
  ...
  return {
    ...
    notifications: [],
    source: `fallback:be_portal_pickup_requests / ${role} / ${normalizedLogin}`,
  };
}
  ...
  async function forgotPassword(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/password-recovery", {
        ...
        body: JSON.stringify({ email: normalizedEmail, mode: "rider" }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        const code = String(result?.error || "");
        throw new Error(code === "rate_limited" ? "Please wait and try again." : code === "invalid_email" ? "Please enter a valid email address." : "Could not send reset email.");
      }
