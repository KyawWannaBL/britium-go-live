...
export default function MustChangePasswordClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const recoveryRaw = window.sessionStorage.getItem("britium-recovery-session");
        if (recoveryRaw) {
          window.sessionStorage.removeItem("britium-recovery-session");
          const recovery = JSON.parse(recoveryRaw);
          const { error: sessionError } = await supabase.auth.setSession({
            access_token: String(recovery?.access_token || ""),
            refresh_token: String(recovery?.refresh_token || ""),
          });
        ...
        if (active) {
          setError(sessionError instanceof Error ? sessionError.message : "Unable to activate the password-reset session.");
        }
      } finally {
