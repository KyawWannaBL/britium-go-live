...
async function overlayLiveDataEntryWaybillRows(rows: AnyRow[]) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      delivery_fee: detail.effective_declared_delivery_charge ?? detail.delivery_charges ?? detail.delivery_fee ?? row.delivery_fee,
      source_waybill_no: detail.financial_quote?.source_waybill_no || row.source_waybill_no,
