...
import { allocateVans, assignCrews, type Resource, type Stop, type VanPlan } from "@/lib/multiVanPlanner";
...
export default function MultiVanPlanner({rows,region,onSaved}:{rows:Stop[];region:string;onSaved:()=>void}) {
 ...
 const changed=(next:VanPlan[])=>{setPlans(next);setApproved(false);setReason("");request.current=null;};
 function preview(){
   try {
     const raw=allocateVans(scopedRows,vehicles,count?Number(count):undefined);
     changed(assignCrews(raw,drivers,helpers,convertMyanmarTownshipToEnglish));
     setMessage("Review the suggested vans and crews. You may adjust assignments before creating.");
   } catch(e:any){setMessage(e.message);}
 }
 async function save(){
   setBusy(true);setMessage("");
   const payload={region_code:region,plans:plans.map(p=>({vehicle_code:p.vehicle_code,driver_code:p.driver_code,helper_code:p.helper_code,delivery_way_ids:p.rows.map(r=>r.delivery_way_id)})),approve_below_minimum:approved,below_minimum_reason:reason};
   ...
   try {
     ...
     setMessage(data.wayplans.length+" Wayplans created for "+data.parcel_count+" parcels.");
 ...
 return <section style={{padding:16,border:"1px solid #1a3a5c",borderRadius:16,background:"#0b2236",display:"grid",gap:12}}>
   ...
   <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
     ...
     <button style={button} disabled={busy||!context||!scopedRows.length} onClick={preview}>Preview automatic Wayplans</button>
   </div>
   {message&&<p role="status" style={{margin:0}}>{message}</p>}
   {plans.map((plan,i)=><section key={i} style={{padding:12,border:"1px solid #38566b",borderRadius:10}}>
     <strong>Van {i+1}: {plan.rows.length} parcels · {Array.from(new Set(plan.rows.map(r=>r.township))).join(", ")}</strong>
     ...
     <details style={{marginTop:10}}><summary>Review parcels / move to another van</summary>
       <div style={{maxHeight:280,overflow:"auto"}}>
         {plan.rows.map(row=><div key={row.delivery_way_id} style={{display:"flex",justifyContent:"space-between",gap:10,padding:5}}>
   ...
   {plans.length>0&&<button style={button} disabled={busy||plans.some(p=>!p.driver_code||!p.rows.length)||short.length>1||(short.length===1&&!approved)} onClick={save}>{busy?"Creating Wayplans…":"Create reviewed Wayplans"}</button>}
 </section>;
}
