...
mkdir -p backups
cp src/components/BulkTemplateWorkbench.tsx "backups/BulkTemplateWorkbench.before-alignment-masterdata.$(date +%Y%m%d-%H%M%S).tsx" 2>/dev/null || true

cat > src/components/BulkTemplateWorkbench.tsx <<'TSX'
...
# Remove any old page-level language toggles so only App header remains.
for p in [Path("src/components/BulkTemplateWorkbench.tsx"), Path("src/components/OperationalModuleDashboard.tsx")]:
    if not p.exists():
        continue
    s = p.read_text(encoding="utf-8")
    s = s.replace('import LanguageToggle from "./LanguageToggle";\n', '')
    s = s.replace('import LanguageToggle from "../components/LanguageToggle";\n', '')
    s = s.replace('<LanguageToggle />', '')
    p.write_text(s, encoding="utf-8")

# Tighten account header layout.
p = Path("src/App.tsx")
if p.exists():
    s = p.read_text(encoding="utf-8")
    if ".be-user-actions" not in s:
        marker = "        .be-user span, .be-muted {"
        add = """
        .be-user-actions {
          display: flex;
          align-items: center;
...
"""
        ...
        s = s.replace(".be-user-actions button {\n          padding: 8px 12px;\n          min-width: auto;\n        }", ".be-user-actions button {\n          height: 38px;\n          min-height: 38px;\n          padding: 0 12px;\n          min-width: auto;\n          border-radius: 12px;\n        }")
    p.write_text(s, encoding="utf-8")
PY

echo "Patched template UI alignment, backend dropdown/masterdata loading, and single language toggle cleanup."
