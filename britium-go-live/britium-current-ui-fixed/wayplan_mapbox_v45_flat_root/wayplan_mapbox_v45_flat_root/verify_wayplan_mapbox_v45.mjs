...
for (const token of ["WAYPLAN_V45_SUPERVISOR_MAPBOX_ROUTE_GATE_2026-07-30", "be_wayplan_supervisor_decide_v45"]) requireText(supervisor, token, "Supervisor source");
console.log("PASS source: src/pages/SupervisorWayplanReviewPage.tsx");

const rider = read("src/pages/RiderAppPage.tsx");
for (const token of ["RiderMapboxRouteV45", "wayplanId="]) requireText(rider, token, "Rider source");
console.log("PASS source: src/pages/RiderAppPage.tsx");

const planner = read("src/components/wayplan/MapboxWayplanPlannerV45.tsx");
for (const token of ["FIXED HEAD OFFICE ORIGIN", "Optimize from Head Office", "be_wayplan_save_mapbox_route_v45", "VITE_MAPBOX_ACCESS_TOKEN"]) requireText(planner, token, "Mapbox planner");
...
const bundles = fs.readdirSync(distAssets).filter((name) => name.endsWith(".js")).map((name) => fs.readFileSync(path.join(distAssets, name), "utf8")).join("\n");
for (const token of [
  "WAYPLAN_V45_MAPBOX_HEAD_OFFICE_ROUTE_2026-07-30",
...
console.log("PASS production bundle: Wayplan, Supervisor and Rider Mapbox V45 markers");
console.log("SAFE TO DEPLOY WAYPLAN MAPBOX V45");
