...
const DATA_ENTRY_BUILD = 'DATA_ENTRY_V18_PHOTO_REVIEW_FULL_REGISTER_2026-07-29';
      ...
      const uploadedRows = await Promise.all(
        matrix
          ...
          .map(async (sourceRow, index) => {
            ...
            const wayId = String(getCell(sourceRow, 'Way ID') || existing?.way_id || `UPLOAD-${sequence}`).trim();
            ...
            // Spreadsheet-calculated fields are never trusted; backend/local tariff recalculates them.
            return calculateAmounts(row);
          }),
      );

      if (!uploadedRows.length) throw new Error('No parcel rows found in the uploaded file.');

      setRows((currentRows) => {
        const nextRows = [...currentRows];
        uploadedRows.forEach((uploadedRow, index) => {
          const byWayId = nextRows.findIndex((row) => row.way_id === uploadedRow.way_id);
          const targetIndex = byWayId >= 0 ? byWayId : index;
        ...
        return nextRows.filter(Boolean).sort((a, b) => a.parcel_sequence - b.parcel_sequence);
      });

      setMessage(`Uploaded ${uploadedRows.length} parcel row(s). Delivery charge, extra-kilo charge, and collection amount were recalculated.`);
    } catch (error: any) {
      console.error(error);
