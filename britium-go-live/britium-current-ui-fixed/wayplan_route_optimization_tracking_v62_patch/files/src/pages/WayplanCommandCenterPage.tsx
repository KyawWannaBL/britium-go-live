...
import MapboxWayplanPlannerV45 from "@/components/wayplan/MapboxWayplanPlannerV45";
import WayplanRiderTrackingMapV62 from "@/components/wayplan/WayplanRiderTrackingMapV62";

const BUILD_MARKER = "WAYPLAN_COMMAND_ROUTE_OPTIMIZATION_TRACKING_V62_2026_08_02";
...
function routeDistance(value: any) {
  const meters = Math.max(0, Number(value || 0));
  return meters >= 1000 ? `${(meters / 1000).toFixed(meters >= 10000 ? 0 : 1)} km` : `${Math.round(meters)} m`;
}

function routeDuration(value: any) {
  const minutes = Math.max(0, Math.round(Number(value || 0) / 60));
  const hours = Math.floor(minutes / 60);
  return hours ? `${hours} hr ${minutes % 60} min` : `${Math.max(1, minutes)} min`;
}

function routeEta(value: any) {
  if (!value) return "-";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "-";
  return date.toLocaleString("en-GB", { timeZone: "Asia/Yangon", day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit", hour12: true });
}
...
function normalizeAssignmentOption(row: Row, entity: "rider" | "driver" | "helper" | "vehicle"): AssignmentOption {
  ...
  return {
    ...
    dispatch_eligible: row.dispatch_eligible === undefined ? true : Boolean(row.dispatch_eligible),
    eligibility_reason: text(row.eligibility_reason || meta.eligibility_reason),
  };
}
...
function buildManifestHtml(wayplan: Row, stops: Row[]) {
  ...
  const rows = stops.map((row, index) => `
    <tr>
      ...
      <td>${escapeHtml(routeDistance(row.route_leg_distance_m || row.leg_distance_m))}</td>
      <td>${escapeHtml(routeDuration(row.route_leg_duration_s || row.leg_duration_s))}</td>
      <td>${escapeHtml(routeEta(row.route_eta_at || row.eta_at))}</td>
...
<body>
  ...
  <table>
    <thead>
      <tr><th>#</th><th>Way ID</th><th>Recipient</th><th>Phone</th><th>Township</th><th>Address</th><th>COD</th><th>Kg</th><th>Leg Distance</th><th>Travel</th><th>ETA</th><th>Remarks</th><th>Signature</th></tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
...
export default function WayplanCommandCenterPage() {
  ...
  const [optimizedRoutePlan, setOptimizedRoutePlan] = useState<Row | null>(null);
  const handleMapboxRouteState = useCallback((ready: boolean, state?: Row) => {
    setMapboxRouteReady(Boolean(ready));
    setOptimizedRoutePlan(ready && state ? state : null);
  }, []);
  ...
  const [assignmentMode, setAssignmentMode] = useState<"RIDER" | "VEHICLE_CREW">("RIDER");
  ...
  const loadAssignmentOptions = useCallback(async (silent = false) => {
    ...
    setAssignmentError("");
    try {
      const primary = await supabase.rpc("be_wayplan_assignment_options_v44");
      ...
      setAssignmentError("Using the legacy Master Data dropdown API. Run the V44 SQL to enable exclusive assignment modes and full fleet visibility.");
    } catch (caught: any) {
  ...
  function clearRiderAssignment() {
    setRiderMasterKey("");
    setRiderCode("");
    setRiderName("");
  }

  function clearVehicleCrewAssignment() {
    setVehicleMasterKey("");
    setVehicleCode("");
    setVehicleName("");
    setDriverMasterKey("");
  ...
  function switchAssignmentMode(mode: "RIDER" | "VEHICLE_CREW") {
    setAssignmentMode(mode);
    setError("");
    if (mode === "RIDER") clearVehicleCrewAssignment();
    else clearRiderAssignment();
  }

  function chooseRider(recordKey: string) {
    if (assignmentMode !== "RIDER") switchAssignmentMode("RIDER");
  ...
  function chooseDriver(recordKey: string) {
    if (assignmentMode !== "VEHICLE_CREW") switchAssignmentMode("VEHICLE_CREW");
  ...
  function chooseVehicle(recordKey: string) {
    ...
    if (option.dispatch_eligible === false) {
      setVehicleMasterKey("");
      setError(option.eligibility_reason || `${option.master_vehicle_type || option.vehicle_type || "This vehicle type"} is visible from Master Data but is not permitted for Dispatch.`);
      return;
    }
    ...
    if (option.assigned_driver_id) {
      ...
      if (assignedDriver) {
        setDriverMasterKey(assignedDriver.record_key);
        setDriverCode(assignedDriver.id);
  ...
  const orderedManifestStops = useMemo(() => {
    const ordered = Array.isArray(optimizedRoutePlan?.ordered_stops) ? optimizedRoutePlan.ordered_stops : [];
    if (!ordered.length) return manifestStops;
    const source = new Map(manifestStops.map((row) => [wayId(row), row]));
    const seen = new Set<string>();
    const result = ordered.map((stop: Row, index: number) => {
      const id = wayId(stop);
      const row = source.get(id) || stop;
      seen.add(id);
      return {
        ...row,
        route_sequence: Number(stop.sequence || index + 1),
        route_leg_distance_m: Number(stop.leg_distance_m || 0),
        route_leg_duration_s: Number(stop.leg_duration_s || 0),
        route_cumulative_distance_m: Number(stop.cumulative_distance_m || 0),
        route_cumulative_duration_s: Number(stop.cumulative_duration_s || 0),
        route_eta_at: stop.eta_at || null,
      };
    });
    manifestStops.forEach((row) => { if (!seen.has(wayId(row))) result.push(row); });
    return result;
  }, [manifestStops, optimizedRoutePlan]);

  const activeAssignmentMode = text(activeWayplan?.assignment_mode || activeWayplan?.assignmentMode, assignmentMode).toUpperCase();
  const activeVehicleType = text(activeWayplan?.vehicle_type || activeWayplan?.assigned_vehicle_type, vehicleType);
  const activeRiderCode = text(activeWayplan?.rider_code || activeWayplan?.assigned_rider_code || activeWayplan?.rider_id, riderCode);
  const activeRiderName = text(activeWayplan?.rider_name || activeWayplan?.assigned_rider_name, riderName);
  const activeRiderEmail = text(activeWayplan?.rider_email || activeWayplan?.assigned_rider_email);
  ...
  async function createWayplan() {
    ...
    if (assignmentMode === "RIDER") {
      if (!(riderCode.trim() || riderName.trim())) {
        setError("Rider Delivery mode requires an authorized Rider.");
        return;
      }
    } else {
      if (!(vehicleCode.trim() || vehicleName.trim())) {
        setError("Vehicle Crew mode requires a permitted Vehicle.");
        return;
      }
      if (!(driverCode.trim() || driverName.trim())) {
        setError("Vehicle Crew mode requires an authorized Driver.");
        return;
      }
      if (!(helperCode.trim() || helperName.trim())) {
        setError("Vehicle Crew mode requires an assigned Helper.");
        return;
      }
    }
    ...
    try {
      ...
      const payload = {
        ...
        vehicle_type: assignmentMode === "VEHICLE_CREW" ? vehicleType : null,
        vehicle_code: assignmentMode === "VEHICLE_CREW" ? (vehicleCode.trim() || null) : null,
        vehicle_name: assignmentMode === "VEHICLE_CREW" ? (vehicleName.trim() || null) : null,
        vehicle_master_key: assignmentMode === "VEHICLE_CREW" ? (vehicleMasterKey || null) : null,
        rider_code: assignmentMode === "RIDER" ? (riderCode.trim() || null) : null,
        rider_name: assignmentMode === "RIDER" ? (riderName.trim() || null) : null,
        rider_master_key: assignmentMode === "RIDER" ? (riderMasterKey || null) : null,
        driver_code: assignmentMode === "VEHICLE_CREW" ? (driverCode.trim() || null) : null,
        driver_name: assignmentMode === "VEHICLE_CREW" ? (driverName.trim() || null) : null,
        driver_master_key: assignmentMode === "VEHICLE_CREW" ? (driverMasterKey || null) : null,
        helper_code: assignmentMode === "VEHICLE_CREW" ? (helperCode.trim() || null) : null,
        helper_name: assignmentMode === "VEHICLE_CREW" ? (helperName.trim() || null) : null,
        helper_master_key: assignmentMode === "VEHICLE_CREW" ? (helperMasterKey || null) : null,
        actor,
      };

      const { data, error: rpcError } = await supabase.rpc("be_generate_wayplan_from_warehouse_v44", {
        p_payload: payload,
      });
      ...
      const synthetic = {
        ...
        assignment_mode: assignmentMode,
        vehicle_type: assignmentMode === "VEHICLE_CREW" ? vehicleType : null,
        vehicle_code: assignmentMode === "VEHICLE_CREW" ? vehicleCode : null,
        vehicle_name: assignmentMode === "VEHICLE_CREW" ? vehicleName : null,
        rider_code: assignmentMode === "RIDER" ? riderCode : null,
        rider_name: assignmentMode === "RIDER" ? riderName : null,
        driver_code: assignmentMode === "VEHICLE_CREW" ? driverCode : null,
        driver_name: assignmentMode === "VEHICLE_CREW" ? driverName : null,
        helper_code: assignmentMode === "VEHICLE_CREW" ? helperCode : null,
        helper_name: assignmentMode === "VEHICLE_CREW" ? helperName : null,
        stops: selectedRows,
      ...
      setNotice(`${createdId || "Wayplan"} created in ${assignmentMode === "RIDER" ? "Rider Delivery" : "Vehicle Crew"} mode with ${selectedRows.length} parcel(s). Next: arrange and save the Mapbox route from Head Office before Supervisor review.`);
      await loadAll(true);
    } catch (caught: any) {
  ...
  }, [activeWayplan?.wayplan_id, activeWayplan?.wayplan_code]);
  ...
  async function submitForSupervisorReview() {
    ...
    if (!mapboxRouteReady) {
      setError("Arrange and save the complete Mapbox route from Britium Head Office before Supervisor review.");
      return;
    }
    ...
    try {
      const actor = await actorEmail();
      const { data, error: rpcError } = await supabase.rpc("be_wayplan_submit_review_v45", {
        p_wayplan_id: id,
        p_actor_email: actor,
        p_notes: "Route group, Master Data assignment, and fixed Head Office Mapbox sequence submitted from Wayplan Command V45",
      });
      if (rpcError) throw rpcError;
      ...
      <div style={{ maxWidth: 1540, margin: "0 auto", display: "grid", gap: 16 }}>
        <section style={cardStyle({ padding: 18 })}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
            <div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}><div style={{ color: C.gold, fontSize: 11, fontWeight: 900, letterSpacing: "0.19em" }}>WAYPLAN COMMAND · {BUILD_MARKER}</div><span data-wayplan-master-dropdowns="active" style={{ border: `1px solid ${C.green}`, color: C.green, borderRadius: 999, padding: "3px 8px", fontSize: 9, fontWeight: 950 }}>V44 ASSIGNMENT MODES ACTIVE</span></div>
              <h1 style={{ margin: "7px 0 3px", fontSize: 25 }}>Warehouse Ready → Assignment Mode → Wayplan → Supervisor Approval → Dispatch Scan <span style={{ color: C.green, fontSize: 13, verticalAlign: "middle" }}>V43.1</span></h1>
              <p style={{ margin: 0, color: C.sub, fontSize: 12 }}>
                Warehouse receiving is complete. Choose exactly one assignment model: Rider Delivery, or Vehicle Crew (Vehicle + Driver + Helper). Master Data IDs and names populate automatically; blank rows remain available for controlled manual entry.
              </p>
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 980 }}>
                ...
                <tbody>
                  {filteredRows.map((row) => {
                    ...
                    return (
                      <tr key={`${pickupId(row)}-${id}`} onClick={() => toggleOne(row)} style={{ borderTop: `1px solid ${C.border}`, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.58 : 1, background: selected[id] ? "rgba(246,184,75,0.08)" : "transparent" }}>
          ...
          <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
            <section style={cardStyle()}>
              ...
              <div style={{ ...cardStyle({ padding: 10, background: C.panel2 }), marginBottom: 10 }}>
                <div style={{ color: assignmentError ? C.amber : C.green, fontSize: 10, fontWeight: 900, display: "flex", gap: 7, alignItems: "center" }}>
                  ...
                  {assignmentError || `Master Data API connected · ${assignmentOptions.riders.length} riders · ${assignmentOptions.drivers.length} drivers · ${assignmentOptions.helpers.length} helpers · ${assignmentOptions.vehicles.length} active fleet records · ${assignmentOptions.vehicles.filter((item) => item.dispatch_eligible !== false).length} dispatch eligible`}
                </div>
                <div style={{ color: C.sub, fontSize: 9, marginTop: 4 }}><PencilLine size={11} style={{ verticalAlign: "middle", marginRight: 4 }} />Choose one assignment mode. The other group is cleared automatically. Blank rows unlock manual ID and name entry.</div>
              </div>

              <div style={{ marginBottom: 10 }}>
                <label style={{ color: C.sub, fontSize: 10, fontWeight: 900 }}>Assignment Mode
                  <select value={assignmentMode} onChange={(event) => switchAssignmentMode(event.target.value as "RIDER" | "VEHICLE_CREW")} style={inputStyle({ marginTop: 4 })}>
                    <option value="RIDER">Rider Delivery — Rider only</option>
                    <option value="VEHICLE_CREW">Vehicle Crew — Vehicle + Driver + Helper</option>
                  </select>
                </label>
                <div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 10, border: `1px solid ${assignmentMode === "RIDER" ? C.green : C.blue}`, color: assignmentMode === "RIDER" ? C.green : C.blue, background: C.panel2, fontSize: 10, fontWeight: 900 }}>
                  {assignmentMode === "RIDER"
                    ? "Rider Delivery active: Vehicle, Driver, and Helper are not required."
                    : "Vehicle Crew active: Rider is not required; Vehicle, Driver, and Helper are required."}
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9 }}>
                ...
                <label style={{ color: C.sub, fontSize: 10, fontWeight: 800, gridColumn: "1 / -1" }}>Cut-off<input type="time" value={cutoff} onChange={(event) => setCutoff(event.target.value)} style={inputStyle({ marginTop: 4 })} /></label>
                ...
                {assignmentMode === "RIDER" ? <>
                  ...
                  <div style={{ color: C.sub, fontSize: 9, alignSelf: "end", paddingBottom: 10 }}>All active fleet records are shown. Policy-blocked vehicle types are visible but disabled.</div>
                  ...
                  <label style={{ color: C.sub, fontSize: 10, fontWeight: 800, gridColumn: "1 / -1" }}>Vehicle Master Data / Manual
                    <select value={vehicleMasterKey} onChange={(event) => chooseVehicle(event.target.value)} style={inputStyle({ marginTop: 4 })}>
                      <option value="">— Blank / type manually —</option>
                      {assignmentOptions.vehicles.map((item) => <option key={item.record_key} value={item.record_key} disabled={item.dispatch_eligible === false}>{item.label || `${item.name} · ${item.id}`}{item.dispatch_eligible === false ? " · POLICY BLOCKED" : ""}</option>)}
                    </select>
                  </label>
                  ...
                  <label style={{ color: C.sub, fontSize: 10, fontWeight: 800 }}>Driver ID<input value={driverCode} onChange={(event) => setDriverCode(event.target.value)} readOnly={Boolean(driverMasterKey)} placeholder="Required" style={{ ...inputStyle({ marginTop: 4 }), background: driverMasterKey ? "#dbeafe" : "#f8fafc" }} /></label>
                  <label style={{ color: C.sub, fontSize: 10, fontWeight: 800 }}>Driver Name<input value={driverName} onChange={(event) => setDriverName(event.target.value)} readOnly={Boolean(driverMasterKey)} placeholder="Required" style={{ ...inputStyle({ marginTop: 4 }), background: driverMasterKey ? "#dbeafe" : "#f8fafc" }} /></label>
                  ...
                  <label style={{ color: C.sub, fontSize: 10, fontWeight: 800 }}>Helper ID<input value={helperCode} onChange={(event) => setHelperCode(event.target.value)} readOnly={Boolean(helperMasterKey)} placeholder="Required" style={{ ...inputStyle({ marginTop: 4 }), background: helperMasterKey ? "#dbeafe" : "#f8fafc" }} /></label>
                  <label style={{ color: C.sub, fontSize: 10, fontWeight: 800 }}>Helper Name<input value={helperName} onChange={(event) => setHelperName(event.target.value)} readOnly={Boolean(helperMasterKey)} placeholder="Required" style={{ ...inputStyle({ marginTop: 4 }), background: helperMasterKey ? "#dbeafe" : "#f8fafc" }} /></label>
                </>}
              </div>
            ...
            <section style={cardStyle()}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}>
                <button onClick={() => activeWayplan && printManifest(activeWayplan, orderedManifestStops)} disabled={!activeWayplan || !orderedManifestStops.length} style={{ ...buttonStyle("plain"), opacity: !activeWayplan || !orderedManifestStops.length ? 0.45 : 1 }}><Printer size={15} />Print Manifest</button>
                <button onClick={() => void submitForSupervisorReview()} disabled={!activeWayplan || !mapboxRouteReady || !!actionBusy || ["PENDING_REVIEW", "APPROVED", "DISPATCH_READY", "DISPATCHED"].includes(text(reviewState.review_status))} style={{ ...buttonStyle("green"), opacity: !activeWayplan || !mapboxRouteReady || actionBusy || ["PENDING_REVIEW", "APPROVED", "DISPATCH_READY", "DISPATCHED"].includes(text(reviewState.review_status)) ? 0.45 : 1 }}>{actionBusy === "review" ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />}Submit for Review</button>
              ...
              <div style={{ marginTop: 10, color: C.sub, fontSize: 10, lineHeight: 1.6 }}>
                Mapbox must save the complete stop sequence from Britium Head Office before Supervisor review. After approval, the Supervisor sends the Wayplan to Dispatch, where every parcel must still be scanned before Publish.
              </div>
            </section>
          </div>
        </div>
        ...
        <MapboxWayplanPlannerV45
          wayplanId={text(activeWayplan?.wayplan_id || activeWayplan?.wayplan_code)}
          fallbackStops={manifestStops}
          assignmentMode={activeAssignmentMode}
          vehicleType={activeVehicleType}
          routeDate={text(activeWayplan?.route_date, routeDate)}
          onRouteStateChange={handleMapboxRouteState}
        />

        {mapboxRouteReady ? <WayplanRiderTrackingMapV62
          wayplanId={text(activeWayplan?.wayplan_id || activeWayplan?.wayplan_code)}
          riderCode={activeRiderCode}
          riderName={activeRiderName}
          riderEmail={activeRiderEmail}
          assignmentMode={activeAssignmentMode}
          vehicleType={activeVehicleType}
        /> : null}
        ...
        <section style={cardStyle()}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
            <div>
              ...
              <div style={{ color: C.sub, fontSize: 11, marginTop: 3 }}>{text(activeWayplan?.wayplan_id || activeWayplan?.wayplan_code, "No Wayplan selected")} · {orderedManifestStops.length} parcel(s)</div>
            </div>
          ...
          <div style={{ overflow: "auto", marginTop: 10, border: `1px solid ${C.border}`, borderRadius: 12 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1050 }}>
              <thead style={{ background: C.panel3 }}><tr>{["Seq", "Way ID", "Recipient", "Phone", "Township", "Address", "COD", "Weight", "Leg Distance", "Travel", "ETA", "Remarks"].map((heading) => <th key={heading} style={{ textAlign: "left", padding: 9, color: C.sub, fontSize: 10 }}>{heading}</th>)}</tr></thead>
              <tbody>
                {orderedManifestStops.map((row, index) => (
                  <tr key={`${wayId(row)}-${index}`} style={{ borderTop: `1px solid ${C.border}` }}>
                    ...
                    <td style={{ padding: 9, color: C.blue, fontWeight: 850 }}>{routeDistance(row.route_leg_distance_m || row.leg_distance_m)}</td>
                    <td style={{ padding: 9 }}>{routeDuration(row.route_leg_duration_s || row.leg_duration_s)}</td>
                    <td style={{ padding: 9, color: C.green, fontWeight: 850 }}>{routeEta(row.route_eta_at || row.eta_at)}</td>
                    <td style={{ padding: 9 }}>{text(row.remarks || row.remark, "-")}</td>
                  </tr>
                ))}
                {!orderedManifestStops.length ? <tr><td colSpan={12} style={{ padding: 28, textAlign: "center", color: C.sub }}>Create or select a Wayplan to preview its manifest.</td></tr> : null}
              </tbody>
            </table>
          </div>
        </section>
