...
import CertifiedOperationalReportingV52 from '@/components/reporting/CertifiedOperationalReportingV52';

const G='#D4AF37'; const N='#0A1628';
type Mode='operational'|'finance'|'certified';
    ...
    <div style={{maxWidth:1050,margin:'0 auto',padding:'24px 20px 60px'}}>
      ...
      <div style={{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap',marginBottom:24,padding:'16px 20px',background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12}}>
        <div style={{display:'flex',gap:4}}>
          {(['operational','finance','certified'] as Mode[]).map(m=>(
        ...
        {mode!=='certified'&&<div style={{display:'flex',gap:8,alignItems:'center'}}>
        ...
        {mode!=='certified'&&<button onClick={run} disabled={loading} style={{background:loading?'rgba(255,255,255,0.08)':G,color:loading?'rgba(255,255,255,0.3)':N,border:'none',borderRadius:8,padding:'8px 22px',fontSize:12,fontWeight:700,cursor:loading?'default':'pointer',marginLeft:'auto'}}>
          {loading?'Loading…':'Generate Report'}
        </button>}
      </div>

      {mode==='certified'&&<CertifiedOperationalReportingV52 />}
