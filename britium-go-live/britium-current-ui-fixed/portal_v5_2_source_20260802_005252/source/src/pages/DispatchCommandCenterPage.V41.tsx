...
const BUILD_MARKER = 'DISPATCH_V41_WAYPLAN_SCANNING_GUARDED_RELEASE_2026-07-30';
...
export default function DispatchCommandCenterPage() {
  ...
  const publish = async (wayplanCode?: string) => {
    ...
    try {
      const email = await actor();
      const { data, error } = await supabase.rpc("be_dispatch_publish_wayplan_v41", {
        p_wayplan_id: code,
        p_actor_email: email,
      });
      if (error) throw error;
      ...
      setMessage(e.message || "Publish stopped. Scan every parcel in the selected Wayplan first.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="mb-4 rounded-xl border border-cyan-800 bg-[#0B2133] p-4">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">Only parcels belonging to the selected V40 Wayplan are accepted. Every parcel must be scanned before the guarded Publish action releases the route to the assigned Rider. {BUILD_MARKER}</p>
