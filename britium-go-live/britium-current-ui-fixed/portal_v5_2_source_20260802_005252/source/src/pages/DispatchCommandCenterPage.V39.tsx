...
const BUILD_MARKER = 'DISPATCH_V39_MANDATORY_SCAN_AUTO_RTO_2026-07-30';
...
export default function DispatchCommandCenterPage() {
  ...
  const [dispatchScanMessage, setDispatchScanMessage] = useState('Dispatch scan is mandatory before a parcel can be published or moved Out for Delivery.');
  ...
  const dispatchScannedCount = dispatchScans.filter((row: any) => row.scanned).length;
  ...
  const loadAll = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setSnapshot(data || {});
      const scanIds = [...new Set((data?.jobs || []).map((job: any) => String(getTracking(job) || '').trim()).filter(Boolean))];
      if (scanIds.length) {
        const { data: scanData, error: scanError } = await supabase.rpc('be_dispatch_scan_snapshot_v39', { p_way_ids: scanIds });
        if (!scanError) setDispatchScans(Array.isArray(scanData?.rows) ? scanData.rows : []);
      } else {
        setDispatchScans([]);
      }
  ...
  const scanForDispatch = async (wayId?: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('be_dispatch_scan_parcel_v39', {
        p_way_id: target,
      ...
      if (error) throw error;
      setDispatchScanCode('');
      setDispatchScanMessage(`${target} scanned for Dispatch. Warehouse receiving scan may have been skipped, but this dispatch scan is mandatory.`);
  ...
  const publish = async (wayplanCode?: string) => {
    ...
    const releaseJobs = jobs.filter((job: any) => String(job.wayplan_code || '') === String(code));
    const wayIds = [...new Set(releaseJobs.map((job: any) => String(getTracking(job) || '').trim()).filter(Boolean))];
    if (!wayIds.length) return setMessage(`No parcel rows are available for ${code}.`);
    setLoading(true);
    try {
      const email = await actor();
      const { data, error } = await supabase.rpc("be_publish_wayplan_with_dispatch_scan_v39", {
        p_wayplan_code: code,
        p_way_ids: wayIds,
        p_actor_email: email,
      });
      if (error) throw error;
      setMessage(`Published ${code} after mandatory dispatch-scan validation (${Number(data?.published_rows || wayIds.length)} parcels).`);
      await loadAll();
    } catch (e: any) {
      setMessage(e.message || "Publish stopped. Scan every parcel for Dispatch first.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const jobCard = (j: any) => (
    <div key={getTracking(j)} className="rounded-xl border border-slate-700 bg-[#071827] p-3 shadow-sm border-l-4 border-l-[#C09B30]">
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        <button disabled={!dispatchScanMap.get(String(getTracking(j)))} onClick={() => setJobStatus(getTracking(j), "OUT_FOR_DELIVERY")} className="rounded bg-blue-700 px-2 py-1 text-xs text-white disabled:cursor-not-allowed disabled:opacity-40">Out</button>
        ...
        <button onClick={() => setJobStatus(getTracking(j), "ATTEMPTED_FAILED")} className="rounded bg-amber-700 px-2 py-1 text-xs text-white">Fail Attempt</button>
    ...
    <div className="min-h-screen bg-[#07111e] p-6 text-slate-100">
      <div className="mb-4 flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-slate-400">Dispatch Command Center · Mandatory Dispatch Scan · Automatic 3-Failure RTO</div>
          </div>
        </div>
      ...
      <section className="mb-4 rounded-xl border border-cyan-800 bg-[#0B2133] p-4">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h3 className="m-0 flex items-center gap-2 font-semibold text-cyan-200"><ScanLine className="h-5 w-5" /> Mandatory Dispatch Scan</h3>
            <p className="mt-1 text-xs text-slate-400">Warehouse receiving scan may be optional, but each parcel must be scanned here before Publish or Out for Delivery. {BUILD_MARKER}</p>
          </div>
          <form onSubmit={(event) => { event.preventDefault(); void scanForDispatch(); }} className="flex min-w-0 flex-1 gap-2 xl:max-w-2xl">
            <input value={dispatchScanCode} onChange={(event) => setDispatchScanCode(event.target.value)} placeholder="Scan or type parcel Way ID" className="h-11 min-w-0 flex-1 rounded-lg border border-cyan-800 bg-[#071827] px-3 font-mono text-sm font-bold outline-none focus:border-cyan-400" />
            <button type="submit" disabled={dispatchScanBusy} className="rounded-lg bg-cyan-400 px-4 py-2 text-sm font-black text-[#04111d] disabled:opacity-50">{dispatchScanBusy ? <Loader2 className="mr-1 inline h-4 w-4 animate-spin" /> : <ScanLine className="mr-1 inline h-4 w-4" />}Scan</button>
          </form>
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
          <span className="rounded-full border border-emerald-700 px-3 py-1 text-emerald-300"><ShieldCheck className="mr-1 inline h-3.5 w-3.5" />Scanned {dispatchScannedCount}/{jobs.length}</span>
          <span className="rounded-full border border-amber-700 px-3 py-1 text-amber-300">Remaining {Math.max(jobs.length - dispatchScannedCount, 0)}</span>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-5 lg:grid-cols-10">
        {[
          ...
          ["TO SCAN", Math.max(jobs.length - dispatchScannedCount, 0)],
