...
const BUILD = 'PORTAL_DATA_ENTRY_CLEAN_REVIEW_V61_4_1_INSTALL_2026_08_03';
...
try {
  ...
  if (!directorySource.includes('TOWNSHIP_TARIFF_DIRECTORY') || !directorySource.includes('MMR013019')) {
    fail('The reviewed V61 township directory is required before V61.4.1.', {
  ...
  for (const [rel, accepted] of Object.entries(manifest.accepted_current_canonical_hashes)) {
    ...
    if (!accepted.includes(actual)) {
      fail('Current Data Entry page is not the reviewed V61.3.3 or V61.4 source.', {
        file: rel,
  ...
  const backupRoot = path.join(root, 'production_backups', `data_entry_clean_review_v61_4_1_${stamp}`);
