...
-- Build: FINANCIAL_V2_TOWNSHIP_ALIAS_V61_4_2026_08_03
...
begin
  ...
  if coalesce((select mutation_mode from public.be_data_entry_financial_v2_runtime_v58 where singleton),'') <> 'MUTATION_SHADOW' then
    raise exception 'ABORT: expected MUTATION_SHADOW before V61.4.';
  end if;
end
$preflight$;

create table if not exists public.be_financial_v2_function_backup_v61_4 (
...
insert into public.be_financial_v2_function_backup_v61_4(
  build,function_signature,function_definition,definition_md5
)
select
  'PRE_FINANCIAL_V2_TOWNSHIP_ALIAS_V61_4_2026_08_03',
...
where not exists (
  select 1
  from public.be_financial_v2_function_backup_v61_4
  where build='PRE_FINANCIAL_V2_TOWNSHIP_ALIAS_V61_4_2026_08_03'
);

create or replace function public.be_financial_v2_township_key_v61_4(p_value text)
returns text
language plpgsql
immutable
parallel safe
...
begin
  ...
  v := case
    when v in ('north dagon','dagon myothit north','north dagon myothit') then 'north dagon'
    when v in ('south dagon','dagon myothit south','south dagon myothit') then 'south dagon'
    when v in ('east dagon','dagon myothit east','east dagon myothit') then 'east dagon'
    when v in ('dagon seikkan','dagon myothit seikkan','seikkan dagon') then 'dagon seikkan'
...
comment on function public.be_financial_v2_township_key_v61_4(text)
is 'V61.4 canonical township alias resolver. It normalizes punctuation, Township/Tsp suffixes and approved Yangon alias pairs without changing tariff amounts.';
...
begin
  ...
  where public.be_financial_v2_township_key_v61_4(t.township) = public.be_financial_v2_township_key_v61_4(p_township)
  ...
  return jsonb_build_object(
    ...
    'calculation_version', 'PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_4',
    'resolved_tariff_township', v_tariff.township,
    'resolved_township_key', public.be_financial_v2_township_key_v61_4(p_township),
    'calculated_at', now()
  );
end
$function$;
...
is 'V61.4: V61.3.3 pass-through financial rules preserved; approved township aliases resolve to one active same-tier tariff shape. No tariff amount is invented or changed.';
...
begin
  ...
  if v_alias->>'validation_status' <> 'OK' then
    raise exception 'ABORT: North Dagon STANDARD calculation failed: %',v_alias;
  end if;
  ...
  if (v_standard->>'base_tariff')::bigint <> (v_alias->>'base_tariff')::bigint
     or (v_standard->>'included_kg')::numeric <> (v_alias->>'included_kg')::numeric
     or (v_standard->>'extra_per_kg')::bigint <> (v_alias->>'extra_per_kg')::bigint then
    raise exception 'ABORT: North Dagon aliases do not resolve to the same tariff: % / %',v_standard,v_alias;
  end if;
...
select
  'FINANCIAL_V2_TOWNSHIP_ALIAS_V61_4',
  'function',
  ...
  jsonb_build_object(
    ...
    'north_dagon_aliases',jsonb_build_array('Dagon Myothit (North)','Dagon Myothit North','North Dagon')
  ),
  now()
