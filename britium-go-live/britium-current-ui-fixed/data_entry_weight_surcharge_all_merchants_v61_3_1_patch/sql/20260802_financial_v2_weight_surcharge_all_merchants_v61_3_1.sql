...
create temporary table pg_temp.be_v61_3_1_state (
...
insert into pg_temp.be_v61_3_1_state(singleton,core_before_md5,schema_before_md5,wrapper_before_md5,mutation_mode)
select true,
...
begin
  ...
  return jsonb_build_object(
    ...
    'calculation_version', 'PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_3_1',
    'calculated_at', now()
  );
end
$function$;
...
is 'V61.3.1: tier-specific STANDARD/ROYAL/COMMITMENT tariff calculation for registered and truly unregistered merchants; receiver COD includes customer-paid weight/CBM/other delivery surcharges for item+delivery and delivery-only types.';
...
begin
  ...
  return jsonb_build_object(
    'ok', true,
    'build', 'DATA_ENTRY_FINANCIAL_V2_ALL_MERCHANTS_V61_3_1_2026_08_02',
    'generated_at', now(),
    'data', jsonb_build_object(
      'schema_version', 'FINANCIAL_V2_SCHEMA_V61_3_1_2026_08_02',
...
begin
  foreach v_tier in array array['STANDARD','ROYAL','COMMITMENT'] loop
    select * into v_tariff
    from public.be_parcel_tariffs_v2 t
    where lower(btrim(t.township))='insein'
      and t.customer_tier=v_tier
      and t.status='ACTIVE'
      and t.effective_from <= public.be_business_date()
      and (t.effective_to is null or t.effective_to >= public.be_business_date())
    order by t.effective_from desc
    limit 1;
    if not found then raise exception 'ABORT: Insein active tariff missing for tier %.',v_tier; end if;
    ...
    if v_result->>'validation_status' <> 'OK'
       ...
       or v_result->>'calculation_version' <> 'PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_3_1' then
      raise exception 'ABORT: tier % verification failed: %',v_tier,v_result;
    end if;

    if v_tier='STANDARD' then update pg_temp.be_v61_3_1_state set standard_result=v_result where singleton;
    elsif v_tier='ROYAL' then update pg_temp.be_v61_3_1_state set royal_result=v_result where singleton;
    else update pg_temp.be_v61_3_1_state set commitment_result=v_result where singleton;
    end if;
  end loop;
  ...
  if exists(select 1 from public.be_merchant_financial_profiles_v2 where upper(btrim(merchant_id))='UNREGISTERED_V61_3_1_TEST') then
  ...
  v_unregistered := public.be_data_entry_financial_v2_calculate(jsonb_build_object(
    'merchant_id','UNREGISTERED_V61_3_1_TEST',
  ...
  update pg_temp.be_v61_3_1_state set unregistered_result=v_unregistered where singleton;
  ...
  if v_core_def not like '%PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_3_1%'
     or v_schema_def not like '%FINANCIAL_V2_SCHEMA_V61_3_1_2026_08_02%'
     or v_wrapper_def not like '%UNREGISTERED_MERCHANT_OPERATOR_SELECTION%'
     or v_wrapper_def not like '%MERCHANT_PROFILE_INACTIVE_OR_BLOCKED%' then
    raise exception 'ABORT: deployed source markers are incomplete.';
  end if;
  update pg_temp.be_v61_3_1_state
...
select auth.uid(),null,current_user,
  'FINANCIAL_V2_ALL_MERCHANT_TIER_SUPPORT_DEPLOYED','FUNCTION_SET','FINANCIAL_V2_V61_3_1',
  jsonb_build_object(
    'request_id','FINANCIAL-V2-ALL-MERCHANTS-V61-3-1-20260802',
  ...
  'FINANCIAL-V2-ALL-MERCHANTS-V61-3-1-20260802','FINANCIAL_V2_CONTROL','FUNCTION_SET','FINANCIAL_V2_V61_3_1',
  jsonb_build_object(
    'build','FINANCIAL_V2_ALL_MERCHANTS_V61_3_1_2026_08_02',
    'core_before_md5',(select core_before_md5 from pg_temp.be_v61_3_1_state where singleton),
    'core_after_md5',(select core_after_md5 from pg_temp.be_v61_3_1_state where singleton),
    'schema_before_md5',(select schema_before_md5 from pg_temp.be_v61_3_1_state where singleton),
    'schema_after_md5',(select schema_after_md5 from pg_temp.be_v61_3_1_state where singleton),
    'wrapper_before_md5',(select wrapper_before_md5 from pg_temp.be_v61_3_1_state where singleton),
    'wrapper_after_md5',(select wrapper_after_md5 from pg_temp.be_v61_3_1_state where singleton),
    'historical_rows_changed',false,'financial_v2_mutation_mode_changed',false
  )
where not exists(
  select 1 from public.be_audit_events a
  where a.upload_code='FINANCIAL-V2-ALL-MERCHANTS-V61-3-1-20260802'
     or a.details->>'request_id'='FINANCIAL-V2-ALL-MERCHANTS-V61-3-1-20260802'
);
...
select jsonb_pretty(jsonb_build_object(
  ...
  'standard_result',(select standard_result from pg_temp.be_v61_3_1_state where singleton),
  'royal_result',(select royal_result from pg_temp.be_v61_3_1_state where singleton),
  'commitment_result',(select commitment_result from pg_temp.be_v61_3_1_state where singleton),
  'unregistered_result',(select unregistered_result from pg_temp.be_v61_3_1_state where singleton),
  ...
  'next_gate','INSTALL_BUILD_AND_DEPLOY_V61_3_1_FRONTEND'
));
