...
import { AlertTriangle, CheckCircle2, Loader2, RefreshCw, Search, WalletCards } from "lucide-react";
...
export default function FinanceMerchantSettlementPage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase.from("be_v_finance_merchant_settlement_queue_v2").select("*").order("calculated_at", { ascending: false }).limit(1000);
  ...
  async function settleSelected() {
    ...
    const { data, error } = await supabase.rpc("be_finance_settle_batch_v2", {
    ...
    if (error) setMessage(error.message);
      ...
      setMessage(`Settlement completed. Batch ${data?.settlement_batch_id || batchId}`);
      setSelected({});
      await load();
    }
    setBusy(false);
    ...
    <div className="mx-auto max-w-[1600px] space-y-4">
      <header className="rounded-2xl border border-[#1a3a5c] bg-[#0b2236] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><div className="text-xs font-black uppercase tracking-[0.18em] text-[#f6b84b]">Finance · Parcel Financial V2</div><h1 className="mt-1 text-2xl">Merchant / Online Seller Settlement</h1><p className="text-xs text-[#9cc2d9]">Settle delivered parcels from stored tariff snapshots. Customer collection, company revenue and merchant settlement stay separate.</p></div>
      ...
      <section className="rounded-2xl border border-[#1a3a5c] bg-[#0b2236] p-4">
        <div className="grid gap-2 md:grid-cols-[1.4fr_1fr_1fr_auto_auto]">
          ...
          <button disabled={busy || !selectedRows.length} onClick={settleSelected} className="flex items-center justify-center gap-2 rounded-xl bg-[#f6b84b] px-4 py-2 text-xs font-black text-[#061524] disabled:opacity-40">{busy?<Loader2 className="animate-spin" size={15}/>:<WalletCards size={15}/>}Settle Selected</button>
        </div>
