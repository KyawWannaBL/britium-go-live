...
const sourceCandidates = [
  path.join(root, 'DataEntryPage.V21.tsx'),
  path.join(root, 'DataEntryPage.tsx'),
];
const sourcePath = sourceCandidates.find((candidate) => fs.existsSync(candidate));
if (!sourcePath) throw new Error('DataEntryPage.V21.tsx was not found beside package.json.');
...
for (const marker of required) {
  if (!replacement.includes(marker)) throw new Error(`V21 source is missing marker: ${marker}`);
}
...
for (const target of targets) {
  ...
  if (fs.existsSync(target)) {
    const backup = `${target}.pre-v21.bak`;
...
console.log('Installed Data Entry V21 into:');
...
console.log('Next: remove dist and node_modules/.vite, build, then run verify_data_entry_v21.mjs.');
