...
import { canonicalWayIdFromRow, extractWayIds, WAY_ID_SCAN_V47_BUILD } from "@/lib/wayIdScanV47";
...
const WAYBILL_STUDIO_BUILD = "WAYBILL_STUDIO_V47_CANONICAL_BARCODE_QR_INTEGRITY_2026-07-30";
...
function docNo(row: PrintRow, type: DocType) {
  if (type === "INVOICE") return text(row, "invoice_no", "invoiceNo", "delivery_way_id", "tracking_no", "waybill_no") || "INV-PENDING";
  if (type === "DOCUMENT") return text(row, "document_no", "documentNo", "delivery_way_id", "tracking_no", "waybill_no") || "DOC-PENDING";

  // The visible Way ID, barcode value, and QR value must be the same canonical
  // parcel identifier. Batch Waybill numbers are deliberately excluded.
  return canonicalWayIdFromRow(row) || text(row, "delivery_way_id", "way_id", "tracking_code", "tracking_no") || "INVALID-WAY-ID";
}
...
export default function BritiumUnifiedPrintStudioV37() {
    ...
    try {
      ...
      if (pickupId) {
        ...
        if (details.length) {
          ...
          const detailWayIds = details.map((row: PrintRow) => canonicalWayIdFromRow(row) || text(row, "delivery_way_id", "way_id", "tracking_code", "tracking_no"));
          const canonicalByWayId = await loadCanonicalParcels(detailWayIds);
          const next = details.map((row: PrintRow, index: number) => {
            const rawParcelNo = text(row, "delivery_way_id", "way_id", "tracking_code", "tracking_no");
            const parcelNo = canonicalWayIdFromRow(row) || extractWayIds(rawParcelNo)[0] || "";
          ...
          setMessage(`${next.length} parcel waybill row(s) loaded for pickup ${pickupId}${batchWaybillNo ? ` · Batch ${batchWaybillNo}` : ""}. Barcode/QR integrity uses one canonical Way ID per parcel. ${WAY_ID_SCAN_V47_BUILD}`);
          return;
        }
      }
  ...
  async function guardedPrint(targetRows: PrintRow[]) {
    ...
    if (docType === "WAYBILL") {
      const ids = targetRows.map((row) => canonicalWayIdFromRow(row));
      const invalidRows = ids.filter((id) => !id).length;
      const duplicateRows = ids.length - new Set(ids.filter(Boolean)).size;
      if (invalidRows > 0) {
        setMessage(`Print blocked. ${invalidRows} row(s) do not contain a canonical parcel Way ID such as D0728-CTM-026. Refresh the data and correct those rows before printing.`);
        return;
      }
      if (duplicateRows > 0) {
        setMessage(`Print blocked. ${duplicateRows} duplicate parcel Way ID row(s) were detected. Each printed barcode and QR must identify exactly one parcel.`);
        return;
      }
    }
