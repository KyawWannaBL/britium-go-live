...
const WAYBILL_STUDIO_BUILD = "WAYBILL_STUDIO_V37_RELIABLE_BULK_PRINT_2026-07-30";
...
function normalized(row: PrintRow, type: DocType) {
  ...
  return {
    no: docNo(row, type),
    merchant: text(row, "merchant_name", "merchantName", "os", "sender_name", "merchant_code") || "Britium Merchant",
    ...
    remarks: text(row, "remarks", "remark", "data_entry_remark", "notes", "special_instructions"),
...
export default function BritiumUnifiedPrintStudioV37() {
        ...
        if (details.length) {
          ...
          const next = details.map((row: PrintRow, index: number) => {
            ...
            return {
              ...
              merchant_name: text(row, "merchant_name", "os", "sender_name", "merchant_code") || text(summary || {}, "merchant_name", "merchant_code") || "Britium Merchant",
              recipient_phone: text(row, "recipient_phone", "contact_no_1", "receiver_phone"),
              recipient_address: text(row, "recipient_address", "delivery_address", "receiver_address"),
              township: text(row, "township", "destination_township"),
              delivery_fee: amount(row, "delivery_fee", "delivery_charges"),
              surcharge: amount(row, "surcharge", "extra_weight_charge"),
              actual_collect: amount(row, "actual_collect", "collect_amount", "cod_amount"),
              remarks: text(row, "remarks", "remark", "data_entry_remark"),
            };
          });
  ...
  async function authorizeRows(rowsToAuthorize: PrintRow[], reason: string, printWindow: Window) {
    type AuthorizationOutcome = { row: PrintRow; result?: any; error?: unknown };
    const outcomes: AuthorizationOutcome[] = new Array(rowsToAuthorize.length);
    let cursor = 0;
    let completed = 0;
    const stage = reason ? "Submitting reprint authorization…" : "Authorizing secure print job…";

    const worker = async () => {
      while (true) {
        const index = cursor;
        cursor += 1;
        if (index >= rowsToAuthorize.length) return;

        const row = rowsToAuthorize[index];
        ...
        try {
          ...
          outcomes[index] = { row, error };
        }

        completed += 1;
        setPrintProgress({ stage, completed, total: rowsToAuthorize.length });
        updatePrintWindowProgress(printWindow, stage, completed, rowsToAuthorize.length);
      }
    };

    const workerCount = Math.min(PRINT_AUTH_CONCURRENCY, Math.max(1, rowsToAuthorize.length));
  ...
  async function guardedPrint(targetRows: PrintRow[]) {
    ...
    // Open synchronously while the click is still a trusted browser gesture.
    // Opening after hundreds of awaited authorization calls is blocked by browsers.
    ...
    setPrinting(true);
    setPrintProgress({ stage: "Authorizing secure print job…", completed: 0, total: targetRows.length });
    setMessage(`Preparing ${targetRows.length} ${docType.toLowerCase()} row(s) for printing…`);

    try {
      const firstPass = await authorizeRows(targetRows, "", printWindow);
      const allowed: PrintRow[] = [];
      const needReason: PrintRow[] = [];
      const blocked: string[] = [];

      for (const outcome of firstPass) {
        const documentNo = docNo(outcome.row, docType);
        if (outcome.error) {
          const errorMessage = outcome.error instanceof Error ? outcome.error.message : String(outcome.error || "authorization failed");
          blocked.push(`${documentNo}: ${errorMessage}`);
        } else if (outcome.result?.allowed) {
          allowed.push(outcome.row);
        } else if (outcome.result?.reason_required) {
          needReason.push(outcome.row);
        } else {
          blocked.push(`${documentNo}: ${outcome.result?.message || outcome.result?.reason || "blocked"}`);
        }
      }

      if (needReason.length) {
        const reason = window.prompt(`${needReason.length} ${docType}(s) were printed before. Enter one reprint reason for this batch:`, "");
        if (reason?.trim()) {
          const reprintPass = await authorizeRows(needReason, reason.trim(), printWindow);
          for (const outcome of reprintPass) {
            const documentNo = docNo(outcome.row, docType);
            if (outcome.error) {
              const errorMessage = outcome.error instanceof Error ? outcome.error.message : String(outcome.error || "reprint authorization failed");
              blocked.push(`${documentNo}: ${errorMessage}`);
            } else if (outcome.result?.allowed) {
              allowed.push(outcome.row);
            } else {
              blocked.push(`${documentNo}: ${outcome.result?.message || outcome.result?.reason || "reprint approval requested"}`);
            }
          }
        } else {
      ...
      if (!allowed.length) {
        const detail = blocked.length
          ...
          : "No rows were authorized for printing.";
        showPrintWindowMessage(printWindow, "Nothing was authorized to print", detail);
        setMessage(`Print job stopped: 0 of ${targetRows.length} row(s) were authorized.`);
        return;
      }

      setPrintProgress({ stage: "Building printable pages…", completed: allowed.length, total: allowed.length });
      updatePrintWindowProgress(printWindow, "Building printable pages…", allowed.length, allowed.length);
      printRows(allowed, printWindow);

      const blockedSummary = blocked.length ? ` ${blocked.length} row(s) were not authorized; details are in the browser console.` : "";
      setMessage(`Print window prepared for ${allowed.length} of ${targetRows.length} row(s).${blockedSummary}`);
      if (blocked.length) console.warn("Britium print authorization results", blocked);
    } catch (error) {
