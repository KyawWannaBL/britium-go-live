...
import { describeScanPayload, extractWayIds, WAY_ID_SCAN_V47_BUILD } from "@/lib/wayIdScanV47";
import {
...
const BUILD_MARKER = 'DISPATCH_V47_CANONICAL_WAY_ID_MULTI_SCAN_2026-07-30';
...
export default function DispatchCommandCenterPage() {
  ...
  const processDispatchScanPayload = async (rawPayload: string, source: "single" | "batch" = "single") => {
    const raw = String(rawPayload || "").trim();
    const summary = describeScanPayload(raw);
    if (!raw) return setDispatchScanMessage("Scan or type a parcel Way ID first.");
    if (!summary.ids.length) {
      return setDispatchScanMessage("No valid parcel Way ID was detected. Expected format: D0728-CTM-026.");
    }
    if (!selectedWayplan) return setDispatchScanMessage("Select a prepared Wayplan before scanning.");
    if (scanProcessingRef.current) return;

    scanProcessingRef.current = true;
    if (source === "batch") setBatchScanBusy(true);
    else setDispatchScanBusy(true);
    setDispatchScanMessage("");

    // Clear the hardware-scanner buffer immediately so the next scan cannot be
    // appended to the previous value while the backend request is running.
    if (source === "batch") setBatchScanText("");
    else setDispatchScanCode("");
    ...
    try {
      ...
      const payloadResult = await supabase.rpc("be_dispatch_scan_payload_v47", {
      ...
      // Compatibility fallback: the frontend parser still protects against a
      // concatenated scanner stream even before the V47 SQL is installed.
      if (error) {
        if (summary.ids.length === 1) {
          const fallback = await supabase.rpc("be_dispatch_scan_wayplan_parcel_v41", {
            p_wayplan_id: selectedWayplan,
            p_way_id: summary.ids[0],
            p_actor_email: email,
          });
          if (fallback.error) throw fallback.error;
          data = { accepted: 1, rejected: 0, detected: 1, unique: 1, duplicate_scans: summary.duplicateCount, results: [{ way_id: summary.ids[0], ok: true, result: fallback.data }] };
        } else {
          const fallback = await supabase.rpc("be_dispatch_scan_wayplan_batch_v41", {
            p_wayplan_id: selectedWayplan,
            p_way_ids: summary.ids,
            p_actor_email: email,
          });
          if (fallback.error) throw fallback.error;
          data = { ...fallback.data, accepted: Number(fallback.data?.scanned || 0), rejected: Number(fallback.data?.failed || 0), detected: summary.occurrenceCount, unique: summary.uniqueCount, duplicate_scans: summary.duplicateCount };
        }
      }

      const accepted = Number(data?.accepted ?? data?.scanned ?? 0);
      const rejected = Number(data?.rejected ?? data?.failed ?? 0);
      const detected = Number(data?.detected ?? summary.occurrenceCount);
      const unique = Number(data?.unique ?? summary.uniqueCount);
      const duplicateScans = Number(data?.duplicate_scans ?? summary.duplicateCount);
      const rejectedRows = Array.isArray(data?.results) ? data.results.filter((row: any) => !row?.ok) : [];
      const rejectedPreview = rejectedRows.slice(0, 3).map((row: any) => `${row?.way_id || "Unknown"}: ${row?.error || "Rejected"}`).join(" | ");

      setDispatchScanMessage(
        `Scanner stream: ${detected} code occurrence(s), ${unique} unique. ${accepted} accepted for ${selectedWayplan}, ${rejected} rejected${duplicateScans ? `, ${duplicateScans} duplicate scan(s) ignored` : ""}.${rejectedPreview ? ` ${rejectedPreview}` : ""}`,
      );
      await loadAll(true);
    } catch (error: any) {
      setDispatchScanMessage(error?.message || "Dispatch scan failed.");
    } finally {
      scanProcessingRef.current = false;
      if (source === "batch") setBatchScanBusy(false);
      else setDispatchScanBusy(false);
    }
  };

  const scanForDispatch = async (wayId?: string) => {
    const raw = String(wayId || dispatchScanCode || "");
    await processDispatchScanPayload(raw, "single");
  };

  useEffect(() => {
    if (scanIdleTimer.current) clearTimeout(scanIdleTimer.current);
    if (!selectedWayplan || dispatchScanBusy || !dispatchScanCode.trim()) return;
    if (!extractWayIds(dispatchScanCode).length) return;

    // Keyboard-wedge scanners do not always send Enter. An idle window makes
    // one complete scan self-submit and also captures multi-code camera output.
    scanIdleTimer.current = setTimeout(() => {
      void processDispatchScanPayload(dispatchScanCode, "single");
    }, 240);

    return () => {
      if (scanIdleTimer.current) clearTimeout(scanIdleTimer.current);
    };
  }, [dispatchScanCode, dispatchScanBusy, selectedWayplan]);
  ...
  const scanBatchForDispatch = async () => {
    await processDispatchScanPayload(batchScanText, "batch");
  };
      ...
      <section className="mb-4 rounded-xl border border-cyan-800 bg-[#0B2133] p-4">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">Barcode and QR streams are normalized into canonical parcel Way IDs. Concatenated scans and duplicates are separated safely; only IDs belonging to the selected Wayplan are accepted. Every parcel must be scanned before guarded Publish. {BUILD_MARKER} · {WAY_ID_SCAN_V47_BUILD}</p>
            <form onSubmit={(event) => { event.preventDefault(); void scanForDispatch(); }} className="mt-3 flex min-w-0 gap-2">
              <input autoFocus value={dispatchScanCode} onChange={(event) => setDispatchScanCode(event.target.value)} placeholder="Scan barcode/QR or type parcel Way ID" className="h-11 min-w-0 flex-1 rounded-lg border border-cyan-800 bg-[#071827] px-3 font-mono text-sm font-bold outline-none focus:border-cyan-400" />
          ...
          <div className="rounded-lg border border-slate-700 bg-[#071827] p-3">
            ...
            <textarea value={batchScanText} onChange={(event) => setBatchScanText(event.target.value)} placeholder="Paste scanner output — lines or concatenated Way IDs are supported" className="h-24 w-full resize-y rounded-lg border border-slate-700 bg-[#06131f] p-2 font-mono text-xs outline-none focus:border-cyan-400" />
