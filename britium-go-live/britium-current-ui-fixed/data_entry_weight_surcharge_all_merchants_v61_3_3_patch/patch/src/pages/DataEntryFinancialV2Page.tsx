...
export const DATA_ENTRY_FINANCIAL_V2_BUILD = 'PORTAL_DATA_ENTRY_FINANCIAL_V2_ALL_MERCHANTS_V61_3_3_2026_08_02';
...
export default function DataEntryFinancialV2Page() {
  ...
  return (
    <main className="space-y-5" data-build={DATA_ENTRY_FINANCIAL_V2_BUILD} data-township-directory-build={TOWNSHIP_DIRECTORY_BUILD} data-guided-amount-assistance="V61.3.3" data-all-merchants-tier-support="true" data-merchant-delivery-difference-credit="true" data-weight-surcharge-pass-through="true" lang={language === 'my' ? 'my' : 'en'}>
...
function FieldControl({ field, row, index, language, onInput, onTownshipSelect, onTownshipClear }: {
  ...
  const fieldHelp = field.name === 'additional_customer_charge'
    ...
    : field.name === 'merchant_payable_charges' || field.name === 'other_merchant_credits'
      ...
      : '';
  if (field.name === 'customer_tier') return <label className="text-[10px] font-black uppercase tracking-wider text-[#9cc2d9]">{editableLabel}<select className={INPUT_CLASS} value={text(row.input.customer_tier ?? row.output.customer_tier ?? '')} onChange={(event) => onInput(index, field.name, event.target.value)}><option value="">{tr(language, 'selectCustomerTier')}</option>{CUSTOMER_TIERS.map((item) => <option key={item} value={item}>{item}</option>)}</select><div className="mt-1 text-[9px] font-medium normal-case leading-4 tracking-normal text-[#6f98b8]">{tr(language, 'customerTierHelp')}</div></label>;
