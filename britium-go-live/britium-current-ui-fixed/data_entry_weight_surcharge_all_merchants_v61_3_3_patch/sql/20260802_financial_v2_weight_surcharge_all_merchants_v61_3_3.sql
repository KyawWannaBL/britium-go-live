...
drop table if exists pg_temp.be_v61_3_3_state;
create temporary table pg_temp.be_v61_3_3_state (
...
insert into pg_temp.be_v61_3_3_state(singleton,core_before_md5,schema_before_md5,wrapper_before_md5,mutation_mode)
select true,
...
create or replace function public.be_financial_v2_township_key_v61_3_3(p_value text)
returns text
language sql
immutable
parallel safe
...
comment on function public.be_financial_v2_township_key_v61_3_3(text)
is 'V61.3.3 deterministic township-key normalization: trims spacing and Township/Tsp suffix only.';

-- Persist a narrow backup only for exact Insein tariff rows touched by this migration.
create table if not exists public.backup_be_parcel_tariffs_v61_3_3_20260802
as
select t.*, now()::timestamptz as backed_up_at, null::text as backup_reason
from public.be_parcel_tariffs_v2 t
where false;

insert into public.backup_be_parcel_tariffs_v61_3_3_20260802(
...
select
  ...
  t.effective_from,t.effective_to,t.created_at,t.updated_at,now(),'PRE_V61_3_3_INSEIN_TARIFF_REPAIR'
from public.be_parcel_tariffs_v2 t
where public.be_financial_v2_township_key_v61_3_3(t.township)='insein'
  and not exists(
    select 1 from public.backup_be_parcel_tariffs_v61_3_3_20260802 b where b.id=t.id
  );
...
begin
  foreach v_tier in array array['STANDARD','ROYAL','COMMITMENT'] loop
    ...
    where t.customer_tier=v_tier
      ...
      and public.be_financial_v2_township_key_v61_3_3(t.township) in (
    ...
    if v_peer_count > 0 then
      ...
      order by
        case public.be_financial_v2_township_key_v61_3_3(t.township)
    ...
    insert into public.be_parcel_tariffs_v2(
      ...
      concat_ws(' | ',nullif(v_template.note,''),'V61.3.3 controlled Insein tariff repair copied from active tier-specific 4,500-MMK band'),
  ...
  update pg_temp.be_v61_3_3_state
...
begin
  ...
  from public.be_parcel_tariffs_v2 t
  where public.be_financial_v2_township_key_v61_3_3(t.township) = public.be_financial_v2_township_key_v61_3_3(p_township)
    and t.customer_tier = v_tier
    and t.status = 'ACTIVE'
    and t.effective_from <= public.be_business_date()
    and (t.effective_to is null or t.effective_to >= public.be_business_date())
  ...
  return jsonb_build_object(
    ...
    'calculation_version', 'PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_3_3',
    'calculated_at', now()
  );
end
$function$;
...
is 'V61.3.3: tier-specific STANDARD/ROYAL/COMMITMENT tariff calculation for registered and truly unregistered merchants; receiver COD includes customer-paid weight/CBM/other delivery surcharges for item+delivery and delivery-only types.';
...
begin
  ...
  return jsonb_build_object(
    'ok', true,
    'build', 'DATA_ENTRY_FINANCIAL_V2_ALL_MERCHANTS_V61_3_3_2026_08_02',
    'generated_at', now(),
    'data', jsonb_build_object(
      'schema_version', 'FINANCIAL_V2_SCHEMA_V61_3_3_2026_08_02',
...
begin
  foreach v_tier in array array['STANDARD','ROYAL','COMMITMENT'] loop
    ...
    if v_result->>'validation_status' <> 'OK'
       ...
       or v_result->>'calculation_version' <> 'PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_3_3' then
      raise exception 'ABORT: tier % verification failed: %',v_tier,v_result;
    end if;

    if v_tier='STANDARD' then update pg_temp.be_v61_3_3_state set standard_result=v_result where singleton;
    elsif v_tier='ROYAL' then update pg_temp.be_v61_3_3_state set royal_result=v_result where singleton;
    else update pg_temp.be_v61_3_3_state set commitment_result=v_result where singleton;
    end if;
  end loop;

  -- Supabase SQL Editor does not carry an end-user JWT. Establish a transaction-local
  -- service_role claim only for this migration's authenticated RPC self-tests.
  -- This does not change grants, RLS, runtime mode, or application-user access.
  ...
  if exists(select 1 from public.be_merchant_financial_profiles_v2 where upper(btrim(merchant_id))='UNREGISTERED_V61_3_3_TEST') then
  ...
  v_unregistered := public.be_data_entry_financial_v2_calculate(jsonb_build_object(
    'merchant_id','UNREGISTERED_V61_3_3_TEST',
  ...
  update pg_temp.be_v61_3_3_state set unregistered_result=v_unregistered where singleton;
  ...
  if v_core_def not like '%PARCEL_FINANCIAL_V2_WEIGHT_PASS_THROUGH_V61_3_3%'
     or v_schema_def not like '%FINANCIAL_V2_SCHEMA_V61_3_3_2026_08_02%'
     ...
     or v_core_def not like '%be_financial_v2_township_key_v61_3_3%' then
...
select auth.uid(),null,current_user,
  'FINANCIAL_V2_ALL_MERCHANT_TIER_SUPPORT_DEPLOYED','FUNCTION_SET','FINANCIAL_V2_V61_3_3',
  jsonb_build_object(
    'request_id','FINANCIAL-V2-ALL-MERCHANTS-V61-3-3-20260802',
  ...
  'FINANCIAL-V2-ALL-MERCHANTS-V61-3-3-20260802','FINANCIAL_V2_CONTROL','FUNCTION_SET','FINANCIAL_V2_V61_3_3',
  jsonb_build_object(
    'build','FINANCIAL_V2_ALL_MERCHANTS_V61_3_3_2026_08_02',
    'core_before_md5',(select core_before_md5 from pg_temp.be_v61_3_3_state where singleton),
    'core_after_md5',(select core_after_md5 from pg_temp.be_v61_3_3_state where singleton),
    'schema_before_md5',(select schema_before_md5 from pg_temp.be_v61_3_3_state where singleton),
    'schema_after_md5',(select schema_after_md5 from pg_temp.be_v61_3_3_state where singleton),
    'wrapper_before_md5',(select wrapper_before_md5 from pg_temp.be_v61_3_3_state where singleton),
    'wrapper_after_md5',(select wrapper_after_md5 from pg_temp.be_v61_3_3_state where singleton),
    'tariff_rows_repaired',(select tariff_rows_repaired from pg_temp.be_v61_3_3_state where singleton),
    'historical_rows_changed',false,'financial_v2_mutation_mode_changed',false
  )
where not exists(
  select 1 from public.be_audit_events a
  where a.upload_code='FINANCIAL-V2-ALL-MERCHANTS-V61-3-3-20260802'
     or a.details->>'request_id'='FINANCIAL-V2-ALL-MERCHANTS-V61-3-3-20260802'
);
...
select jsonb_pretty(jsonb_build_object(
  ...
  'insein_tariff_tiers_active',(select count(distinct customer_tier) from public.be_parcel_tariffs_v2 t where public.be_financial_v2_township_key_v61_3_3(t.township)='insein' and t.status='ACTIVE' and t.effective_from<=public.be_business_date() and (t.effective_to is null or t.effective_to>=public.be_business_date())),
  'tariff_rows_repaired',(select tariff_rows_repaired from pg_temp.be_v61_3_3_state where singleton),
  'standard_result',(select standard_result from pg_temp.be_v61_3_3_state where singleton),
  'royal_result',(select royal_result from pg_temp.be_v61_3_3_state where singleton),
  'commitment_result',(select commitment_result from pg_temp.be_v61_3_3_state where singleton),
  'unregistered_result',(select unregistered_result from pg_temp.be_v61_3_3_state where singleton),
  ...
  'next_gate','RUN_V61_3_3_SQL_VERIFIER_THEN_INSTALL_BUILD_AND_DEPLOY_FRONTEND'
));
