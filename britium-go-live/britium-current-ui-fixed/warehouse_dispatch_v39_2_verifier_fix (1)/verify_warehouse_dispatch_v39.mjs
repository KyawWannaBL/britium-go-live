...
const sourceChecks = [
  ...
  },
  {
    ...
    sourceRequired: ["be_warehouse_return_scan_v39", "be_warehouse_lifecycle_alerts_v39", "RETURN_TO_SENDER"],
    bundleRequired: ["be_warehouse_return_scan_v39", "be_warehouse_lifecycle_alerts_v39", "RETURN_TO_SENDER"],
  },
  {
  ...
  },
  {
    ...
    sourceRequired: ["be_record_delivery_failure_v39", "be_record_delivery_success_v39", "payload.operationId"],
    // Production minification renames the local `payload` variable (for example to `e`),
    // so checking the literal source expression `payload.operationId` in dist is invalid.
    // Stable RPC names plus the V39 marker prove the Rider integration survived bundling.
    bundleRequired: ["be_record_delivery_failure_v39", "be_record_delivery_success_v39"],
  },
];
...
for (const check of sourceChecks) {
  ...
  for (const item of check.sourceRequired) {
...
for (const check of sourceChecks) {
  ...
  for (const item of check.bundleRequired) {
