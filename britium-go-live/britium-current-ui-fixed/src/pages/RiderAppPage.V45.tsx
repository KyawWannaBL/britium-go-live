...
const riderAppApi = {
  ...
  reportDeliveryException: async (payload: any) => {
    ...
    return { data: { ...(base.data || {}), v39: failure.data, build: RIDER_V39_BUILD_MARKER }, error: null };
  },
...
function AppShell() {
  ...
  async function executeAction(actionType: string, entityType: string, entityKey: string, payloadFactory: (gps: any) => any) {
    ...
    try {
      ...
      if (actionType === "MARK_DELIVERED" || actionType === "REPORT_DELIVERY_EXCEPTION") {
    ...
    <section style={{ display: "grid", gap: 18 }}>
      ...
      {wayplans.map((wp: any) => (
        <article key={wp.id} style={styles.card}>
          <div style={styles.cardTop}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ marginTop: 8 }}>
                <button style={styles.primaryBtn} disabled={busy} onClick={() => onQuick("START_DELIVERY_ROUTE", "wayplan", wp.id, (gps: any) => ({
                  wayplanId: wp.id,
                  manifestId: Number.isInteger(wp.manifest_id) ? wp.manifest_id : null,
                  manifestVersion: Number(wp.manifest_version || 0),
                  riderId,
                  ...gps,
                }))}>
          ...
          <RiderMapboxRouteV45 wayplanId={safe(wp.wayplan_code || wp.wayplan_no || wp.id)} />
