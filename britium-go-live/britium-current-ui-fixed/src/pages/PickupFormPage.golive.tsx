...
export default function PickupFormPage() {
  ...
  useEffect(() => {
    const handleLangChange = (e: any) => {
      const newLang = e.detail === "my" || e.detail === "mm" ? "my" : "en";
  ...
  // --- STYLING (Matches User Mockups perfectly) ---
  const inputClass = `
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-8">
            ...
            <div>
              ...
              <select name="merchantCode" value={formData.merchantCode} onChange={handleMerchantSelect} disabled={loadingOptions} className={`${inputClass} cursor-pointer ${formData.merchantCode ? 'border-[#f6b84b]' : ''}`}>
                <option value="" className="text-gray-400">{loadingOptions ? t.loading : t.phMerchant}</option>
                {merchants.map(m => (
                  <option key={m.value} value={m.value} className="bg-[#0b2236] text-white py-2">{m.value} - {m.label}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select name="paymentTerms" value={formData.paymentTerms} onChange={handleChange} className={`${inputClass} cursor-pointer`}>
                <option value="COD" className="bg-[#0b2236]">COD</option>
                <option value="Prepaid" className="bg-[#0b2236]">Prepaid</option>
                <option value="Monthly" className="bg-[#0b2236]">Monthly</option>
              </select>
            </div>
            ...
            <div className="md:col-span-1">
              ...
              <select name="requiredVehicle" value={formData.requiredVehicle} onChange={handleChange} className={`${inputClass} cursor-pointer`}>
                <option value="Bike" className="bg-[#0b2236]">Bike</option>
                <option value="Van" className="bg-[#0b2236]">Delivery Van</option>
                <option value="Mini Truck" className="bg-[#0b2236]">Mini Truck</option>
              </select>
            </div>
