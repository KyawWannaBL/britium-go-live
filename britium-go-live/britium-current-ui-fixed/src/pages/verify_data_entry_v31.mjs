...
if (!fs.existsSync(active)) {
  ...
  for (const [token, label] of [
    [marker, 'V31 marker'],
    ['tracking_code: row.way_id', 'tracking-code fallback'],
    ["sender_name: row.os || row.merchant_id || row.customer_id || 'Unknown Sender'", 'sender-name fallback'],
    ['recipient_address: row.delivery_address || null', 'recipient-address fallback'],
    ['data_entry_required_fields_explicit_fix_v31.sql', 'V31 SQL guidance'],
...
console.log('SAFE TO DEPLOY V31');
