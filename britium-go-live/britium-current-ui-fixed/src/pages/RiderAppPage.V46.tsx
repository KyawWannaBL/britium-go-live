...
import { acceptManifest, completeDelivery } from "@/lib/routedDeliveryApi";
...
const RIDER_V39_BUILD_MARKER = 'RIDER_V39_CONSECUTIVE_FAILURE_AUTO_RTO_2026-07-30';
const RIDER_V46_BUILD_MARKER = 'RIDER_V46_HEAD_OFFICE_ROUTE_EXECUTION_2026-07-30';
...
// --- API LAYER ---
async function syncRiderRouteStopV46(payload: any, result: "DELIVERED" | "FAILED" | "RTO", details: any = {}) {
  if (!payload?.wayplanId || !payload?.deliveryWayId) return { data: null, error: null };
  return await (supabase as any).rpc("be_rider_record_stop_result_v46", {
    p_wayplan_id: payload.wayplanId,
    p_delivery_way_id: payload.deliveryWayId,
    p_rider_key: payload.riderCode || payload.riderId || null,
    p_result: result,
    p_operation_id: payload.operationId || buildLocalId("route-stop"),
    p_payload: {
      ...details,
      gps_latitude: payload.gpsLat ?? payload.latitude ?? null,
      gps_longitude: payload.gpsLng ?? payload.longitude ?? null,
      receiver_name: payload.receiverName || null,
      cod_collected: payload.codCollected ?? null,
      photo_url: payload.photoUrl || null,
      signature_url: payload.signatureUrl || null,
      remarks: payload.remarks || null,
      device_timestamp: payload.deviceTimestamp || new Date().toISOString(),
      build: RIDER_V46_BUILD_MARKER,
    },
  });
}
...
const riderAppApi = {
  ...
  snapshot: async (riderId: string) => {
    ...
    const snapshot = data || {};
    const { data: canonicalDeliveries, error: deliveryError } = await supabase
      .from("deliveries")
      .select("id, manifest_id, status, version, tracking_no, delivery_id, way_id, delivered_at, updated_at");

    if (deliveryError || !canonicalDeliveries?.length) return snapshot;

    const manifestIds = [
      ...new Set(
        canonicalDeliveries
          .map((delivery: any) => delivery.manifest_id)
          .filter((id: unknown): id is number => Number.isInteger(id)),
      ),
    ];

    const { data: canonicalManifests } = manifestIds.length
      ? await supabase
          .from("manifests")
          .select("id, status, version, accepted_at")
          .in("id", manifestIds)
      : { data: [] };

    const manifestsById = new Map<any, any>(
      (canonicalManifests || []).map((manifest: any) => [manifest.id, manifest]),
    );
    const deliveriesByKey = new Map<string, any>();
    ...
    for (const delivery of canonicalDeliveries) {
      for (const key of [
        ...
        if (key) deliveriesByKey.set(String(key), delivery);
      }
    }

    return {
      ...snapshot,
      delivery_wayplans: (snapshot.delivery_wayplans || []).map((wayplan: any) => {
        let routeManifestId: number | null = null;
        const stops = (wayplan.stops || []).map((stop: any) => {
          const canonical = [
            stop.delivery_record_id,
            stop.delivery_uuid,
            stop.id,
            stop.tracking_no,
            ...
            .filter(Boolean)
            .map(String)
            .map((key) => deliveriesByKey.get(key))
            .find(Boolean);

          if (!canonical) return stop;
          routeManifestId ||= canonical.manifest_id;

          return {
            ...stop,
            delivery_record_id: canonical.id,
        ...
        const manifest = routeManifestId
          ? manifestsById.get(routeManifestId)
          : null;

        return {
          ...wayplan,
          stops,
          manifest_id: manifest?.id ?? wayplan.manifest_id,
          manifest_version: manifest?.version ?? wayplan.manifest_version,
          manifest_status: manifest?.status ?? wayplan.manifest_status,
        };
      }),
    };
  },
  ...
  startDeliveryRoute: async (payload: any) => {
    if (Number.isInteger(payload.manifestId)) {
      await acceptManifest({
        manifestId: payload.manifestId,
        expectedVersion: Number(payload.manifestVersion || 0),
      });
    }
    if (payload.wayplanId) {
      return await (supabase as any).rpc("be_rider_start_route_v46", {
        p_wayplan_id: payload.wayplanId,
        p_rider_key: payload.riderCode || payload.riderId || null,
        p_latitude: payload.gpsLat ?? payload.latitude,
        p_longitude: payload.gpsLng ?? payload.longitude,
        p_operation_id: payload.operationId || buildLocalId("start-route"),
      });
    }
    return await (supabase as any).rpc("be_rider_start_delivery_route", { p_payload: payload });
  },
  arriveDeliveryStop: async (payload: any) => {
    if (payload.wayplanId) {
      return await (supabase as any).rpc("be_rider_arrive_stop_v46", {
        p_wayplan_id: payload.wayplanId,
        p_delivery_way_id: payload.deliveryWayId,
        p_rider_key: payload.riderCode || payload.riderId || null,
        p_latitude: payload.gpsLat ?? payload.latitude,
        p_longitude: payload.gpsLng ?? payload.longitude,
        p_operation_id: payload.operationId || buildLocalId("arrive-stop"),
      });
    }
    return await (supabase as any).rpc("be_rider_arrive_delivery_stop", { p_payload: payload });
  },

  markDelivered: async (payload: any) => {
    if (payload.deliveryId && payload.operationId) {
      try {
        ...
        await (supabase as any).rpc('be_record_delivery_success_v39', {
          p_way_id: payload.deliveryWayId,
          p_actor_email: payload.riderId || null,
          p_operation_id: payload.operationId,
        });
        const routeSync = await syncRiderRouteStopV46(payload, "DELIVERED", { canonical_delivery: true });
        if (routeSync.error) return routeSync;
        return { data: { ...(data || {}), route_v46: routeSync.data }, error: null };
      } catch (error) {
    ...
    if (!rich.error) {
      ...
      const routeSync = await syncRiderRouteStopV46(payload, "DELIVERED", { rich_delivery: true });
      if (routeSync.error) return routeSync;
      return { data: { ...(rich.data || {}), route_v46: routeSync.data }, error: null };
    }

    // Temporary backward-compatible fallback for environments where the finance/signature SQL has not yet been applied.
    const fallback = await (supabase as any).rpc("be_rider_mark_delivered", { p_payload: payload });
    if (!fallback.error) {
      await (supabase as any).rpc('be_record_delivery_success_v39', {
        p_way_id: payload.deliveryWayId,
        p_actor_email: payload.riderId || null,
      ...
      const routeSync = await syncRiderRouteStopV46(payload, "DELIVERED", { fallback_delivery: true });
      if (routeSync.error) return routeSync;
      return { data: { ...(fallback.data || {}), route_v46: routeSync.data }, error: null };
    }
    return fallback;
  },

  reportDeliveryException: async (payload: any) => {
    const base = await (supabase as any).rpc("be_rider_report_delivery_exception", { p_payload: payload });
    if (base.error) return base;
    const failure = await (supabase as any).rpc('be_record_delivery_failure_v39', {
      p_way_id: payload.deliveryWayId,
      p_reason: [payload.exceptionCode, payload.remarks].filter(Boolean).join(': ') || 'Delivery exception',
      p_actor_email: payload.riderId || null,
      p_operation_id: payload.operationId || null,
    });
    if (failure.error) return failure;
    const result = failure.data?.rto || String(failure.data?.status || "").toUpperCase() === "RTO" ? "RTO" : "FAILED";
    const routeSync = await syncRiderRouteStopV46(payload, result, {
      exception_code: payload.exceptionCode || null,
      failure_v39: failure.data || null,
    });
    if (routeSync.error) return routeSync;
    return { data: { ...(base.data || {}), v39: failure.data, route_v46: routeSync.data, build: RIDER_V46_BUILD_MARKER }, error: null };
  },
...
function AppShell() {
  ...
  async function executeAction(actionType: string, entityType: string, entityKey: string, payloadFactory: (gps: any) => any) {
    ...
    const markCompletionPending = () => {
      setSnapshot((current: any) => ({
        ...current,
        delivery_wayplans: (current?.delivery_wayplans || []).map((wayplan: any) => ({
          ...wayplan,
          stops: (wayplan.stops || []).map((stop: any) => {
            const key = stop.delivery_way_id || stop.tracking_no || stop.id;
            return String(key) === String(entityKey)
              ? { ...stop, status: "COMPLETED_PENDING_SYNC" }
              : stop;
          }),
        })),
      }));
    ...
    try {
      ...
      if (["START_DELIVERY_ROUTE", "ARRIVE_DELIVERY_STOP", "MARK_DELIVERED", "REPORT_DELIVERY_EXCEPTION"].includes(actionType)) {
        payload.operationId ||= buildLocalId();
        payload.deviceTimestamp ||= new Date().toISOString();
      }
      actionPayload = payload;

      if (!hasNavigator() || !navigator.onLine) {
        riderStorage.enqueue(actionType, entityType, entityKey, payload);
        if (actionType === "MARK_DELIVERED") markCompletionPending();
        setQueueSize(riderStorage.getQueue().length);
      ...
      if (
        ...
        actionPayload &&
        isUnknownNetworkOutcome
      ) {
        riderStorage.enqueue(actionType, entityType, entityKey, actionPayload);
        markCompletionPending();
        setQueueSize(riderStorage.getQueue().length);
        setActiveAction(null);
        setMessage({
          type: "warning",
          text: "Completion saved on this device and will sync when connectivity returns.",
        });
        playAlert("alert");
        return;
      }
  ...
  useEffect(() => {
    if (isOnline && riderId && riderStorage.getQueue().length > 0) {
      void flushQueue();
    }
    // Flush only when connectivity or the authenticated rider changes.
  ...
  return (
    <main style={styles.page}>
      <div style={styles.shell}>
        ...
        {tab === "deliveries" ? (
          <DeliveryTab wayplans={wayplans} rules={deliveryRules} rider={rider} riderId={riderId} busy={busy} onAction={setActiveAction} onQuick={executeAction} onRefresh={() => load(riderId)} />
        ) : null}
...
function DeliveryTab({ wayplans, rules, rider, riderId, onAction, onQuick, onRefresh, busy }: any) {
  const { t } = useI18n();
  return (
        ...
        <article key={wp.id} style={styles.card}>
          <div style={styles.cardTop}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ marginTop: 8, color: C.green, fontSize: 10, fontWeight: 950 }}>{RIDER_V46_BUILD_MARKER}</div>
            </div>
          </div>

          <RiderRouteExecutionV46 wayplan={wp} rider={rider} busy={busy} onRefresh={onRefresh} />
              ...
              <div key={s.id} style={styles.stopRow}>
                ...
                <div style={{ display: "grid", gap: 8, justifyItems: "end" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "end" }}>
                    <button style={styles.smallBtn} onClick={() => onQuick("ARRIVE_DELIVERY_STOP", "shipment", s.delivery_way_id, (gps: any) => ({ wayplanId: wp.wayplan_code || wp.wayplan_no || wp.id, deliveryWayId: s.delivery_way_id, riderId, riderCode: rider?.rider_code || rider?.code || null, ...gps }))}>
                      <MapPin size={14} /> {t("arriveDelivery")}
                    </button>
                    <button style={styles.smallPrimaryBtn} onClick={() => onAction({ type: "MARK_DELIVERED", entity: { ...s, wayplan_id: wp.wayplan_code || wp.wayplan_no || wp.id, rider_code: rider?.rider_code || rider?.code || null } })}>
                      <PackageCheck size={14} /> {t("markDelivered")}
                    </button>
                    <button style={styles.smallDangerBtn} onClick={() => onAction({ type: "REPORT_DELIVERY_EXCEPTION", entity: { ...s, wayplan_id: wp.wayplan_code || wp.wayplan_no || wp.id, rider_code: rider?.rider_code || rider?.code || null }, rules })}>
  ...
  async function submit() {
    ...
    if (action.type === "CONFIRM_PICKUP_COMPLETED") {
      ...
      await onSubmit("MARK_DELIVERED", "shipment", e.delivery_way_id, (gps: any) => ({
        wayplanId: e.wayplan_id || e.wayplan_code || null,
        deliveryWayId: e.delivery_way_id || e.tracking_no || e.waybill_id,
        deliveryId: e.delivery_record_id || e.delivery_uuid || null,
        riderCode: e.rider_code || null,
        expectedVersion: Number(e.version || 0),
        riderId,
        receiverName: form.receiverName,
        codCollected: form.codCollected !== "" ? Number(form.codCollected) : null,
      ...
      await onSubmit("REPORT_DELIVERY_EXCEPTION", "shipment", e.delivery_way_id, (gps: any) => ({
        wayplanId: e.wayplan_id || e.wayplan_code || null, deliveryWayId: e.delivery_way_id, riderCode: e.rider_code || null, exceptionCode: form.exceptionCode, riderId, remarks: form.remarks, photoUrl: form.photoUrl || null, callAttemptCount: form.callAttemptCount ? Number(form.callAttemptCount) : null, codNote: form.codNote || null, nextAttemptDate: form.nextAttemptDate || null, ...gps,
      }));
    }
  }
