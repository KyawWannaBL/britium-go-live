...
const BUILD_MARKER = "WAYPLAN_V45_SUPERVISOR_MAPBOX_ROUTE_GATE_2026-07-30";
...
export default function SupervisorWayplanReviewPage() {
  ...
  async function decide(decision: "APPROVE" | "REJECT", sendToDispatch = false) {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("be_wayplan_supervisor_decide_v45", {
        p_wayplan_id: id,
        p_decision: decision,
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#1a3a5c] pb-4">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-xs text-[#9cc2d9]">
            Review the fixed Head Office Mapbox route, one-route-group manifest, Master Data assignments, parcel eligibility, COD exposure, and warehouse holds before mandatory Dispatch scanning.
          </p>
        </div>
