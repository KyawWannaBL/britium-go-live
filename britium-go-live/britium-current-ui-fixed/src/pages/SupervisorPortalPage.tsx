...
function normalizeAssignmentStatus(status?: string) {
  ...
  if (["ACCEPTED", "ACCEPT", "ACCEPTED_PICKUP", "PICKUP_VERIFIED", "DELIVERED", "COD_SETTLED"].includes(value)) {
    return "ACCEPTED";
  }
...
function TeamStatusCard({ label, name, code, status }: { label: string; name?: string; code?: string; status?: string }) {
  const visibleStatus = normalizeAssignmentStatus(status);

  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-3">
      <div className="text-xs text-slate-400">{label}</div>
      <div className="mt-1 truncate font-semibold text-white">{name || code || "Not assigned"}</div>
      <span className={`mt-2 inline-flex rounded-full border px-2 py-1 text-xs font-bold ${statusColor(visibleStatus)}`}>
...
function normalizePickup(row: any): PickupQueueItem {
  ...
  return {
    ...
    driver_status: asText(row.driver_status),
    helper_status: asText(row.helper_status),
    team_acceptance_status: asText(row.team_acceptance_status),
    ...
    assigned_rider_id: asText(row.assigned_rider_id || row.rider_auth_id),
    assigned_driver_id: asText(row.assigned_driver_id || row.driver_auth_id),
    assigned_helper_id: asText(row.assigned_helper_id || row.helper_auth_id),
    ...
    assigned_rider_name: asText(row.assigned_rider_name || row.rider_name),
    assigned_driver_name: asText(row.assigned_driver_name || row.driver_name),
    assigned_helper_name: asText(row.assigned_helper_name || row.helper_name),
...
async function getActorEmail() {
  ...
  if (!data.user?.email) throw new Error("Authenticated supervisor identity is unavailable.");
...
async function loadAssignmentMasters() {
  ...
  const activeWorkers = workforceRows
    ...
    .filter((row) => row.id && (row.code || row.email) && isActiveStatus(row.status));
...
export default function SupervisorPortalPage() {
  ...
  async function loadData(showSpinner = true) {
    ...
    try {
      ...
      const normalizedPickups = (pickups || []).map(normalizePickup);
      const pickupIds = normalizedPickups.map((pickup) => pickup.pickup_id).filter(Boolean);
      ...
      const statusMap = new Map(statusRows.map((row) => [asText(row.pickup_way_id), row]));

      setQueue(
        normalizedPickups.map((pickup) => {
          const liveStatus = statusMap.get(pickup.pickup_id) || {};
          return normalizePickup({
            ...pickup,
            ...liveStatus,
            assigned_rider_name: pickup.assigned_rider_name || liveStatus.assigned_rider_code || liveStatus.assigned_rider_email,
            assigned_driver_name: pickup.assigned_driver_name || liveStatus.assigned_driver_code || liveStatus.assigned_driver_email,
            assigned_helper_name: pickup.assigned_helper_name || liveStatus.assigned_helper_code || liveStatus.assigned_helper_email,
          });
        }),
      );
  ...
  const eligibleFleets = useMemo(() => {
    ...
    return fleets.filter((fleet) => {
      if (!isActiveStatus(fleet.status)) return false;

      const vehicleType = asText(fleet.vehicle_type).toUpperCase().replace(/[^A-Z0-9]+/g, "_");
      if (["VAN", "DELIVERY_VAN", "BIKE", "BICYCLE"].includes(vehicleType)) return true;

      if (["MOTORBIKE", "MOTORCYCLE"].includes(vehicleType)) {
        return Boolean(
          fleet.branch_code &&
          selectedItem.branch_code &&
          asText(fleet.branch_code).toUpperCase() === asText(selectedItem.branch_code).toUpperCase()
        );
      }
  ...
  const assignedCount = queue.filter((item) =>
    Boolean(
      item.assigned_rider_code ||
      item.assigned_driver_code ||
      item.assigned_helper_code ||
      [
  ...
  const pendingCount = queue.filter((item) =>
    [item.supervisor_status, item.pickup_status, item.workflow_stage]
      .map((status) => asText(status).toUpperCase())
      .some((status) => ["PENDING_ASSIGNMENT", "PICKUP_REQUESTED", "WAITING_ASSIGNMENT"].includes(status)),
  ).length;
  ...
  function selectItem(item: PickupQueueItem) {
    ...
    setSelectedRider(
      ...
      riders.find((worker) => [worker.email, worker.code].includes(item.assigned_rider_email || item.assigned_rider_code || ""))?.id ||
      ""
    );
    setSelectedDriver(
      item.assigned_driver_id ||
      drivers.find((worker) => [worker.email, worker.code].includes(item.assigned_driver_email || item.assigned_driver_code || ""))?.id ||
      ""
    );
    setSelectedHelper(
      item.assigned_helper_id ||
      helpers.find((worker) => [worker.email, worker.code].includes(item.assigned_helper_email || item.assigned_helper_code || ""))?.id ||
      ""
    );
  ...
  async function assignJob() {
    ...
    if (!selectedRider) {
      setMessage(t("Please select an authenticated Rider.", "Authenticated Rider တစ်ဦးကို ရွေးချယ်ပါ။"));
      return;
    }

    if (!selectedFleet) {
      setMessage(t("Please select an eligible fleet vehicle.", "သတ်မှတ်ချက်နှင့်ကိုက်ညီသော ယာဉ်ကို ရွေးချယ်ပါ။"));
      return;
    }
    ...
    try {
      ...
      const { data, error } = await (supabase as any).rpc("be_supervisor_assign_job", {
        p_payload: {
          pickup_id: selectedItem.pickup_id,
          pickup_way_id: selectedItem.pickup_id,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_0.8fr] gap-6">
                      ...
                      <div>
                        ...
                        {assigned ? (
                          <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-3">
                            <TeamStatusCard
                              label="Rider"
                              name={item.assigned_rider_name}
                              code={item.assigned_rider_code}
                              status={item.rider_status}
                            />
                            <TeamStatusCard
                              label="Driver"
                              name={item.assigned_driver_name}
                              code={item.assigned_driver_code}
                              status={item.driver_status}
                            />
                            <TeamStatusCard
                              label="Helper"
                              name={item.assigned_helper_name}
                              code={item.assigned_helper_code}
                              status={item.helper_status}
                            />
                          </div>
                        ) : null}
                      </div>
        ...
        <div className="bg-[#0b2236] border border-[#1a3a5c] rounded-3xl p-6 shadow-xl h-fit">
          ...
          <div className="space-y-5">
            ...
            {selectedItem ? (
              <div className="rounded-2xl border border-[#1a3a5c] bg-[#061524] p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[#4d7a9b]">
                    {t("Team Acceptance Status", "အဖွဲ့ လက်ခံမှု အခြေအနေ")}
                  </span>
                  <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase ${statusColor(selectedItem.team_acceptance_status)}`}>
                    {(selectedItem.team_acceptance_status || "WAITING_TEAM_ACCEPTANCE").replace(/_/g, " ")}
                  </span>
                </div>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                  <TeamStatusCard
                    label="Rider"
                    name={selectedItem.assigned_rider_name}
                    code={selectedItem.assigned_rider_code}
                    status={selectedItem.rider_status}
                  />
                  <TeamStatusCard
                    label="Driver"
                    name={selectedItem.assigned_driver_name}
                    code={selectedItem.assigned_driver_code}
                    status={selectedItem.driver_status}
                  />
                  <TeamStatusCard
                    label="Helper"
                    name={selectedItem.assigned_helper_name}
                    code={selectedItem.assigned_helper_code}
                    status={selectedItem.helper_status}
                  />
                </div>
              </div>
            ) : null}
            ...
            <div>
              <span className="block text-[11px] font-bold uppercase tracking-wider text-[#4d7a9b] mb-2">
                <span>{t("Authenticated Rider (required)", "Authenticated Rider (လိုအပ်သည်)")}</span>
              </span>
            ...
            <div>
              ...
              <select value={selectedFleet} onChange={(e) => setSelectedFleet(e.target.value)} className="h-12 w-full rounded-xl bg-[#061524] border border-[#1a3a5c] text-white px-4 text-[13px] font-bold outline-none focus:border-[#f6b84b] cursor-pointer">
                ...
                {eligibleFleets.map((f) => <option key={f.id} value={f.id}>{f.vehicle_no} - {f.vehicle_type || "Vehicle"}</option>)}
              </select>
            </div>
            ...
            <button onClick={assignJob} disabled={loading || !selectedItem || !selectedRider || !selectedFleet} className="w-full h-14 rounded-2xl bg-[#f6b84b] hover:bg-[#e5a93a] text-[#061524] font-black uppercase tracking-wider transition-colors disabled:opacity-50 cursor-pointer shadow-lg shadow-[#f6b84b]/10 flex items-center justify-center gap-2">
