...
const BUILD_MARKER = "WAYPLAN_V42_MASTER_DATA_ROUTE_ASSIGNMENT_2026-07-30";
...
export default function WayplanCommandCenterPage() {
  ...
  async function createWayplan() {
    ...
    try {
      ...
      setSelected({});
      setNotice(`${createdId} created with ${selectedRows.length} parcel(s). Master Data assignments were resolved and saved. Next: prepare it for mandatory Dispatch scanning.`);
      await loadAll(true);
    } catch (caught: any) {
          ...
          <div style={{ display: "flex", justifyContent: "space-between", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
            <div>
              ...
              <p style={{ margin: 0, color: C.sub, fontSize: 12 }}>
                Warehouse receiving is complete. Assign Rider, Driver, Helper, and Vehicle from live Master Data—or use the blank row for controlled manual entry—then continue to mandatory Dispatch scanning.
              </p>
            </div>
            ...
            <section style={cardStyle()}>
              ...
              <div style={{ ...cardStyle({ padding: 10, background: C.panel2 }), marginBottom: 10 }}>
                <div style={{ color: assignmentError ? C.amber : C.green, fontSize: 10, fontWeight: 900, display: "flex", gap: 7, alignItems: "center" }}>
                  ...
                  {assignmentError || `Master Data connected · ${assignmentOptions.riders.length} riders · ${assignmentOptions.drivers.length} drivers · ${assignmentOptions.helpers.length} helpers · ${assignmentOptions.vehicles.length} eligible vehicles`}
                </div>
