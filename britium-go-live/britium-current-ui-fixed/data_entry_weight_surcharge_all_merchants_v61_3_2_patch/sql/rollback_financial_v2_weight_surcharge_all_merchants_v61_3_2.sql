-- Controlled rollback for V61.3.2. Use only after a confirmed regression and only in MUTATION_SHADOW.
begin;
...
-- Restore only exact Insein tariff rows from the V61.3.2 backup.
do $restore_tariff$
begin
  if to_regclass('public.backup_be_parcel_tariffs_v61_3_2_20260802') is not null then
    ...
    select id,township,customer_tier,tariff_zone,tariff_zone_code,base_tariff,included_kg,
      extra_per_kg,commitment_min_ways,commitment_refund_per_way,note,status,
      effective_from,effective_to,created_at,updated_at
    from public.backup_be_parcel_tariffs_v61_3_2_20260802
    order by id
...
$function$;
drop function if exists public.be_financial_v2_township_key_v61_3_2(text);
commit;
select jsonb_pretty(jsonb_build_object('ok',true,'build','ROLLBACK_FINANCIAL_V2_V61_3_2_TO_V61_3_V58_WRAPPER','mutation_mode',(select mutation_mode from public.be_data_entry_financial_v2_runtime_v58 where singleton),'financial_writes_enabled',false));
