...
export const DATA_ENTRY_FINANCIAL_V2_BUILD = 'PORTAL_DATA_ENTRY_FINANCIAL_V2_BILINGUAL_V61_1_2026_08_02';
...
export default function DataEntryFinancialV2Page() {
  ...
  function updateInput(index: number, field: string, value: unknown) {
    commitRows((current) => current.map((row, rowIndex) => rowIndex === index
      ? {
          ...row,
          input: { ...row.input, [field]: value },
          output: clearCalculatedOutput(row.output),
          calculation: undefined,
          saveCheck: undefined,
          error: '',
          selectedTownshipCode: field === 'township' ? undefined : row.selectedTownshipCode,
        }
      : row));
  }
  ...
  return (
    <main className="space-y-5" data-build={DATA_ENTRY_FINANCIAL_V2_BUILD} lang={language === 'my' ? 'my' : 'en'}>
