...
export const DATA_ENTRY_FINANCIAL_V2_BUILD = 'PORTAL_DATA_ENTRY_FINANCIAL_V2_MERCHANT_DELIVERY_CREDIT_V61_2_1_2026_08_02';
...
const UI = {
  en: {
    ...
    positiveDeliveryDifferenceRule: 'A positive delivery difference is credited to the merchant through the backend settlement adjustment. Do not enter it again as an additional customer charge or other merchant credit.',
  ...
  my: {
    ...
    positiveDeliveryDifferenceRule: 'အပေါင်းတန်ဖိုးဖြစ်သော ပို့ဆောင်ခကွာဟချက်ကို ဗဟိုစနစ်က လုပ်ငန်းရှင်သို့ ပေးရန်ငွေအဖြစ် ရှင်းတမ်းညှိနှိုင်းမှုတွင် အလိုအလျောက် ထည့်သွင်းပါသည်။ ယင်းငွေကို ဖောက်သည်ထံမှ ထပ်ဆောင်းကောက်ခံငွေ သို့မဟုတ် လုပ်ငန်းရှင်ရရန်ရှိ အခြားငွေများအကွက်ထဲ ထပ်မထည့်ရပါ။',
...
const FIELD_MY: Record<string, string> = {
  ...
  extra_kg: 'အပိုအလေးချိန်', weight_surcharge: 'အလေးချိန် ထပ်ဆောင်းခ', gross_system_delivery_charge: 'စနစ်တွက် မူလပို့ဆောင်ခ',
  commitment_refund: 'သဘောတူညီချက်အရ ပြန်အမ်းငွေ', net_system_delivery_charge: 'စနစ်တွက် အသားတင်ပို့ဆောင်ခ',
  effective_declared_delivery_charge: 'အတည်ပြု အသုံးပြုမည့် ပို့ဆောင်ခ', delivery_difference: 'ပို့ဆောင်ခ ကွာဟချက်',
...
function amountTypeGuidance(amountType: string, language: UiLanguage): string {
  ...
  switch (amountType) {
    case 'ITEM_PRICE_PLUS_DECLARED_DELIVERY':
      return my
        ? 'စနစ်အကြံပြုချက် — လုပ်ငန်းရှင်သတ်မှတ်စုစုပေါင်း = ကုန်ပစ္စည်းတန်ဖိုး + လုပ်ငန်းရှင်ဖော်ပြပို့ဆောင်ခ။ အပေါင်းတန်ဖိုးဖြစ်သော ပို့ဆောင်ခကွာဟချက်ကို ဗဟိုစနစ်က လုပ်ငန်းရှင်သို့ ပေးရန်ငွေအဖြစ် ရှင်းတမ်းတွင် ထည့်သွင်းပါသည်။ ဖောက်သည်ထံမှ ထပ်ဆောင်းကောက်ခံငွေကို ပို့ဆောင်ခကွာဟချက်မှ အလိုအလျောက်မယူပါ။'
        : 'System suggestion: merchant-stated subtotal = item price + merchant-declared delivery. A positive delivery difference is credited to the merchant by the backend settlement. Additional customer charge is never derived from the delivery difference.';
...
export default function DataEntryFinancialV2Page() {
  ...
  return (
    <main className="space-y-5" data-build={DATA_ENTRY_FINANCIAL_V2_BUILD} data-township-directory-build={TOWNSHIP_DIRECTORY_BUILD} data-guided-amount-assistance="V61.2.1" data-merchant-delivery-difference-credit="true" lang={language === 'my' ? 'my' : 'en'}>
...
function FinancialRowCard({ row, index, fieldsBySection, schema, language, blocked, onInput, onTownshipSelect, onTownshipClear, onCalculate, onSaveCheck, onPhotoReviewed, onPhotoUnavailable }: {
  ...
  return (
    <article className="rounded-3xl border border-[#1a3a5c] bg-[#0b2236] p-5">
      ...
      <MerchantDeliveryDifferenceSummary row={row} language={language} />
...
function MerchantDeliveryDifferenceSummary({ row, language }: { row: EditorRow; language: UiLanguage }) {
  if (!row.calculation?.ok) return null;
  const data = { ...row.output, ...(row.calculation.data || {}) };
  const differenceRaw = data.delivery_difference;
  if (differenceRaw === null || differenceRaw === undefined || differenceRaw === '') return null;
  const difference = numberValue(differenceRaw);
  const direction = text(data.settlement_direction).trim().toUpperCase();
  const adjustmentRaw = data.merchant_settlement_adjustment;
  const finalSettlementRaw = data.merchant_final_settlement_amount;
  const isCredit = difference > 0 || direction === 'CREDIT_TO_MERCHANT';
  const isDebit = difference < 0 || direction === 'DEDUCT_FROM_MERCHANT';
  const amount = Math.abs(difference).toLocaleString('en-US');
  const adjustment = adjustmentRaw === null || adjustmentRaw === undefined || adjustmentRaw === ''
    ? '—'
    : `${numberValue(adjustmentRaw).toLocaleString('en-US')} MMK`;
  const finalSettlement = finalSettlementRaw === null || finalSettlementRaw === undefined || finalSettlementRaw === ''
    ? tr(language, 'merchantCreditPending')
    : `${numberValue(finalSettlementRaw).toLocaleString('en-US')} MMK`;

  if (!isCredit && !isDebit) return null;
  const headline = isCredit ? tr(language, 'merchantDeliveryCredit') : tr(language, 'merchantDeliveryDebit');
  const differenceRule = tr(language, isCredit ? 'positiveDeliveryDifferenceRule' : 'negativeDeliveryDifferenceRule');
  const message = language === 'my'
    ? `${headline}: ${amount} MMK။ ${differenceRule} ရှင်းတမ်းညှိနှိုင်းတန်ဖိုး: ${adjustment}။ ${tr(language, 'merchantFinalSettlementLabel')}: ${finalSettlement}။`
    : `${headline}: ${amount} MMK. ${differenceRule} Settlement adjustment: ${adjustment}. ${tr(language, 'merchantFinalSettlementLabel')}: ${finalSettlement}.`;
  return <Notice tone={isCredit ? 'success' : 'warning'} message={message} />;
}
