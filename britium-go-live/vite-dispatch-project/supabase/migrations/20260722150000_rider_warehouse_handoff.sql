-- Britium rider -> warehouse handoff workflow
-- Rewritten 2026-07-22
-- Security: authenticated/service-role only; SECURITY DEFINER functions verify JWT claims.
-- Proof safety: only uploaded http/https proof URLs are accepted.
-- Partial verification: parcel submissions remain pending until all parcels are approved.
-- Expected JWT metadata: worker_code/account_code, workforce_role/role, and branch_code.
...
declare
  ...
  v_claim_branch text := upper(coalesce(
    nullif(v_claims #>> '{app_metadata,branch_code}', ''),
    nullif(v_claims #>> '{user_metadata,branch_code}', ''),
    ''
  ));
  v_record_branch text := '';
  v_is_admin boolean := false;
  v_is_warehouse boolean := false;
  v_rec public.be_portal_pickup_requests%rowtype;
  v_record jsonb;
  v_event_status text;
  v_title text;
...
begin
  ...
  v_record := to_jsonb(v_rec);
  v_record_branch := upper(coalesce(
    nullif(v_record ->> 'origin_branch_code', ''),
    nullif(v_record ->> 'branch_code', ''),
    nullif(v_record ->> 'pickup_branch_code', ''),
    ''
  ));
  v_is_admin := v_auth_role = 'service_role'
    or v_business_role in (
  ...
  v_is_warehouse := v_business_role in (
    'warehouse',
    'warehouse_staff',
    'warehouse_admin',
    'branch_warehouse',
  ...
  if not v_is_admin and not v_is_warehouse then
    return jsonb_build_object(
      'ok', false,
      'error', 'WAREHOUSE_ROLE_REQUIRED'
    );
  end if;
  ...
  if not v_is_admin
     and v_claim_branch <> ''
     and v_record_branch <> ''
     and v_claim_branch <> v_record_branch then
    return jsonb_build_object(
      'ok', false,
      'error', 'WAREHOUSE_BRANCH_MISMATCH',
      'required_branch', v_record_branch
    );
  end if;

  if v_auth_role <> 'service_role' then
    v_actor_code := coalesce(nullif(v_claim_worker_code, ''), v_actor_code);
    v_actor_name := coalesce(nullif(v_claim_display_name, ''), v_actor_name);
  end if;
...
revoke all on function public.be_rider_pickup_action(jsonb)
from public, anon;

revoke all on function public.be_rider_submit_partial_pickup_verification(jsonb)
from public, anon;

revoke all on function public.be_warehouse_intake_action(jsonb)
from public, anon;

grant execute on function public.be_rider_pickup_action(jsonb)
to authenticated, service_role;

grant execute on function public.be_rider_submit_partial_pickup_verification(jsonb)
to authenticated, service_role;
