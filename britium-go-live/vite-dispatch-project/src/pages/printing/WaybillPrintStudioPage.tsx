...
const PrintPage = ({ children }: { children: React.ReactNode }) => (
  <div
    className="waybill-print-page w-[4in] h-[6in] bg-white mb-6 shadow-xl print:shadow-none print:mb-0 flex flex-wrap overflow-hidden box-border border print:border-none"
...
export default function WaybillPrintStudioPage() {
  ...
  const handlePrint = async () => {
    const firstWaybillNo = generateID(mockData.p1.date, mockData.p1.merchant, 1);

    try {
      await guardedBrowserPrint({
        documentType: 'WAYBILL',
        documentNo: firstWaybillNo,
        actorEmail: 'operator@britiumexpress.com',
        actorRole: 'operator',
        reason: 'Waybill Print Studio print',
      });
    } catch (err: any) {
      alert(err?.message || 'Print blocked. Superadmin approval is required.');
    }
  };

  return (
    <div className="waybill-studio-screen min-h-screen bg-gray-200 text-black font-sans flex flex-col items-center py-4 px-3 print:py-0 print:px-0 print:bg-white">

      {/* GLOBAL PRINT STYLES - Protects the exact 4x6 thermal dimension mapping */}
      <style dangerouslySetInnerHTML={{__html: `
        @page { size: 4in 6in; margin: 0; }

        .waybill-studio-screen {
        ...
        @media print {
            ...
            .waybill-studio-screen { height: auto !important; overflow: visible !important; background: white !important; }
            .waybill-print-page { transform: none !important; margin-bottom: 0 !important; }
