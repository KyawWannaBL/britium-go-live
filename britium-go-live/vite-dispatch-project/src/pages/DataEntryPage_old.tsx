...
export default function DataEntryPage() {
  ...
  try {
    ...
    const queue = (normalizeDataEntryRows(data) as PickupQueueRow[]).map((item: any) => ({
      ...item,
      // keep compatibility with the old Data Entry page fields
    ...
    <div className="space-y-6">
      ...
      <div className={`grid grid-cols-1 ${isFullScreen ? 'lg:grid-cols-1' : 'lg:grid-cols-4'} gap-6 items-start`}>
                  ...
                  <div key={`${proof.pickup_id}-${proof.parcel_sequence}`} className="bg-[#061524] border border-[#1a3a5c] rounded-xl overflow-hidden">
                    ...
                    {proof.photo_url ? (
                      <a href={proof.photo_url} target="_blank" rel="noreferrer" className="block group">
                        <img src={beProofUrl(proof)} alt={proof.delivery_way_id} className="w-full h-32 object-cover border-t border-[#1a3a5c] group-hover:opacity-90" />
        ...
        <div className={`${isFullScreen ? '' : 'lg:col-span-3'} bg-[#0b2236] border border-[#1a3a5c] rounded-2xl flex flex-col overflow-hidden`}>
          ...
          {/* ALWAYS VISIBLE RIDER PHOTO STRIP ABOVE DATA ENTRY FORM */}
          <div className="shrink-0 border-b border-[#1a3a5c] bg-[#071a2b] p-4">
            <div className="flex items-center justify-between gap-3 mb-3">
              <div className="min-w-0">
                <div className="text-[11px] uppercase tracking-widest text-[#f6b84b] font-bold">
                  Rider Photo Verification
                </div>
                <div className="text-[11px] text-[#4d7a9b] mt-1 truncate">
                  {selectedPickup ? (
                    <>
                      ...
                      <span className="mx-2">•</span>
                      Photos: <span className="text-[#f6b84b]">{rows.filter((r: any) => r.photo_url || r.proof_photo_path || r.proof_url).length}</span> / {rows.length}
                    </>
                  ) : (
                    "Select pickup to view rider photos."
                  )}
              ...
              <div className="shrink-0 text-[10px] text-[#4d7a9b] hidden md:block">
                Check rider photos before saving or generating waybill.
              </div>
            </div>
            ...
            <div className="overflow-x-auto custom-scrollbar pb-2">
              <div className="flex gap-3 min-w-max">
                {rows.length === 0 ? (
                  <div className="bg-[#061524] border border-[#1a3a5c] rounded-xl h-32 w-64 flex items-center justify-center text-[#4d7a9b] text-[12px]">
                    No parcel rows loaded
                  </div>
                ) : (
                  rows.map((row: any, index: number) => {
                    const url = row.photo_url || row.proof_photo_path || row.proof_url || "";
                    return (
                      <div key={`${row.delivery_way_id || row.way_id || index}-photo-strip`} className="bg-[#061524] border border-[#1a3a5c] rounded-xl overflow-hidden w-56 shrink-0">
                        <div className="p-2">
                          <div className="text-[#f6b84b] text-[11px] font-semibold truncate">
                            {row.delivery_way_id || row.way_id || `Parcel ${index + 1}`}
                          </div>
                          <div className="text-[#4d7a9b] text-[10px]">
                            #{String(row.parcel_sequence || index + 1).padStart(3, "0")} • {Number(row.weight_kg || row.weight || 0).toLocaleString()} KG
                          </div>
                        </div>
                        {url ? (
                          <a href={url} target="_blank" rel="noreferrer" className="block group">
                            <img
                              src={url}
                              alt={row.delivery_way_id || row.way_id || `Parcel ${index + 1}`}
                              className="w-full h-32 object-cover border-t border-[#1a3a5c] group-hover:opacity-90"
                            />
                            <div className="p-2 text-[10px] text-[#4ea8de]">
                              Open rider photo
                            </div>
                          </a>
                        ) : (
                          <div className="h-32 flex items-center justify-center text-[#4d7a9b] text-[11px] border-t border-[#1a3a5c]">
                            No rider photo
                          </div>
                        )}
                      </div>
