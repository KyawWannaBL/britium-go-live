  ...
  return (
    <section className="rounded-2xl border border-[#1a3a5c] bg-[#0b2236] p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => dryRun(index)} disabled={
            ...
            {row.checking ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} VALIDATE SAVE
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Field label="Recipient name"><input className={inputClass} value={row.recipient_name} onChange={(e) => updateRow(index,{recipient_name:e.target.value})}/></Field>
        <Field label="Recipient phone"><input className={inputClass} value={row.recipient_phone} onChange={(e) => updateRow(index,{recipient_phone:e.target.value})}/></Field>
        <Field label="Township"><input className={inputClass} value={row.township} onChange={(e) => updateRow(index,{township:e.target.value})}/></Field>
        <Field label="Actual weight (kg)"><input type="number" step="0.01" className={inputClass} value={row.weight_kg} onChange={(e)=>updateRow(index,{weight_kg:e.target.value===""?"":Number(e.target.value)})}/></Field>
        <Field label="Recipient address"><textarea rows={2} className={inputClass} value={row.delivery_address} onChange={(e)=>updateRow(index,{delivery_address:e.target.value})}/></Field>
        <Field label="Merchant level">
        ...
        <Field label="Collection method">
          <select className={inputClass} value={row.amount_entry_type} onChange={(e)=> {
            ...
            {AMOUNT_TYPES.map(v=><option key={v} value={v}>{v.replaceAll("_"," ")}</option>)}
          </select>
        </Field>
      </div>

      <div className="mt-4 rounded-xl border border-[#f6b84b]/25 bg-[#1d2b37] p-4">
        <div className="mb-3 text-[10px] font-black uppercase tracking-[0.16em] text-[#f6b84b]">Collection Instructions</div>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {!isExact(type) && type!=="DELIVERY_CHARGE_ONLY" ? <Field label="Item price"><input type="number" className={inputClass} value={row.item_price} onChange={(e)=>updateRow(index,{item_price:e.target.value===""?"":Number(e.target.value)})}/></Field>:null}
          {!isExact(type) && type!=="ITEM_PRICE_ONLY_DELIVERY_PAID_BY_MERCHANT" ? <Field label="Merchant declared delivery"><input type="number" className={inputClass} value={row.delivery_charges} onChange={(e)=>updateRow(index,{delivery_charges:e.target.value===""?"":Number(e.target.value)})}/></Field>:null}
          {isExact(type) ? <Field label="Exact / opaque gross collection"><input type="number" className={inputClass} value={row.merchant_stated_total_amount} onChange={(e)=>updateRow(index,{merchant_stated_total_amount:e.target.value===""?"":Number(e.target.value)})}/></Field>:null}
          <Field label="Additional customer charge"><input type="number" className={inputClass} value={row.additional_customer_charge} onChange={(e)=>updateRow(index,{additional_customer_charge:e.target.value===""?"":Number(e.target.value)})}/></Field>
          <Field label="CBM surcharge"><input type="number" className={inputClass} value={row.cbm_surcharge} onChange={(e)=>updateRow(index,{cbm_surcharge:e.target.value===""?"":Number(e.target.value)})}/></Field>
          <Field label="Other surcharge"><input type="number" className={inputClass} value={row.other_surcharge} onChange={(e)=>updateRow(index,{other_surcharge:e.target.value===""?"":Number(e.target.value)})}/></Field>
        </div>
      </div>

      <div className="mt-4 rounded-xl border border-[#3aa7de]/25 bg-[#071b2b] p-4">
        <div className="mb-3 text-[10px] font-black uppercase tracking-[0.16em] text-[#64c8ff]">Backend Financial Settlement</div>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-6">
          <MoneyBox label="Receiver collection / COD" value={c.cod_amount} highlight />
          ...
          <MoneyBox label="Merchant final settlement" value={c.merchant_final_settlement_amount} highlight />
        </div>
        ...
        <header className="rounded-2xl border border-[#1a3a5c] bg-[#0b2236] p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-black">Pickup Registration & Merchant Settlement</h1>
              <p className="mt-2 max-w-4xl text-[12px] leading-5 text-[#92b7cf]">Backend-authoritative receiver collection, Britium entitlement, delivery difference and merchant final settlement. Live financial persistence remains disabled while the backend mutation gate is shadow.</p>
            </div>
        ...
        <section className="rounded-2xl border border-[#1a3a5c] bg-[#0b2236] p-4">
          <div className="flex flex-wrap items-end gap-3">
            <div className="min-w-[320px] flex-1">
              <div className={labelClass}>Verified pickup request</div>
            ...
            <button type="button" onClick={()=>void loadStartup()} className="inline-flex items-center gap-2 rounded-lg border border-[#3aa7de]/40 bg-[#12314a] px-4 py-2.5 text-[11px] font-black text-[#8fd3ff]"><RefreshCw size={14}/>REFRESH</button>
            <button type="button" onClick={()=>void calculateAll()} disabled={!rows.length} className="inline-flex items-center gap-2 rounded-lg border border-[#34d399]/40 bg-[#0d3b32] px-4 py-2.5 text-[11px] font-black text-[#68e8bd] disabled:opacity-50"><Calculator size={14}/>CALCULATE ALL</button>
            <button type="button" onClick={()=>setFullRegistration(true)} disabled={!rows.length} className="inline-flex items-center gap-2 rounded-lg bg-[#f6b84b] px-4 py-2.5 text-[11px] font-black text-[#061524] disabled:opacity-50"><Maximize2 size={14}/>FULL REGISTRATION</button>
          </div>
      ...
      {fullRegistration?<div data-full-review-sheet="true" className="fixed inset-0 z-[9999] overflow-auto bg-[#04111d]">
        <div className="sticky top-0 z-10 border-b border-[#1a3a5c] bg-[#071b2b]/95 px-5 py-4 backdrop-blur">
          <div className="mx-auto flex max-w-[1900px] items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={()=>void calculateAll()} className="inline-flex items-center gap-2 rounded-lg border border-[#34d399]/40 bg-[#0d3b32] px-4 py-2 text-[11px] font-black text-[#68e8bd]"><Calculator size={14}/>CALCULATE ALL</button>
