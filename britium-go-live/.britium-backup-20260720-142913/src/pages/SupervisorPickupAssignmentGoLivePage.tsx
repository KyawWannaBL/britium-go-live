...
function teamStatus(pickup: Pickup) {
  const statuses = [pickup.rider_status, pickup.driver_status, pickup.helper_status]
    .filter(Boolean)
    .map((status) => normalizeAssignmentStatus(status));

  if (!statuses.length) return "WAITING_TEAM_ACCEPTANCE";
