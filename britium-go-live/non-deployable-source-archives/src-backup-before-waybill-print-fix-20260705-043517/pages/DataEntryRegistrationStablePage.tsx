...
export default function DataEntryRegistrationStablePage() {
  ...
  async function save(submit = false) {
    ...
    try {
      ...
      toast(res.data?.message || (submit ? "Submitted all information." : "Draft saved."));
    } catch (error: any) {
      toast(error?.message || "Unable to save. Run be_data_entry_stable_backend_v5.sql first.");
    } finally {
      setSaving(false);
    }
  }
      ...
      <section style={{ border: `1px solid ${C.border}`, background: C.panel, borderRadius: 20, padding: 18, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ marginTop: 8, color: C.sub }}>Pickup: {selected?.pickup_id || "-"} - Lines: {lines.length} - Rider proof photos: {proofs.filter((p) => p.photo_url).length}/{proofs.length}</div>
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "flex-end" }}>
            ...
            <button style={btn(C.green, "#061524")} onClick={() => save(true)} disabled={saving}><Send size={16} /> Save Data & Generate Waybill</button>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {proofs.length === 0 ? (
              ...
              <button key={`${proof.delivery_way_id || index}-${index}`} type="button" onClick={() => proof.photo_url && setLargePhoto(proof)} style={{ border: `1px solid ${proof.photo_url ? C.green : C.border}`, background: C.panel2, borderRadius: 14, padding: 10, color: C.text, textAlign: "left", cursor: proof.photo_url ? "zoom-in" : "default" }}>
                {proof.photo_url ? <img src={proof.photo_url} style={{ width: "100%", height: 130, objectFit: "cover", borderRadius: 10, display: "block" }} /> : <div style={{ height: 110, display: "grid", placeItems: "center", color: C.sub, border: `1px dashed ${C.border}`, borderRadius: 10 }}>Rider proof pending / no photo saved</div>}
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, gap: 8 }}><span style={{ color: C.sub }}>Parcel {proof.parcel_sequence || index + 1}</span><strong style={{ color: proof.photo_url ? C.green : C.gold }}>{proof.status || proof.photo_state || "PENDING"}</strong></div>
