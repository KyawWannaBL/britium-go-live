...
// --- MOCKED DEPENDENCIES (Removed missing './mapbox_wayplan_routing' import) ---
...
export const BRITIUM_HEAD_OFFICE = {
...
export const orderStopsByHeadOfficeDistance = (stops: WayplanStop[], origin: { latitude: number; longitude: number }) => {
  return stops.map((s) => ({ ...s, distanceFromHeadOfficeKm: 5.5 }))
}

export const optimizeRouteWithMapbox = async (stops: WayplanStop[], token: string, opts: any) => {
  return {
    orderedStops: stops.map((s, i) => ({ ...s, sequence: i + 1, distanceFromHeadOfficeKm: 5.5 })),
    geometry: {},
  }
}

export const buildSupabaseMapboxResultPayload = (optimized: any) => {
  return { mapbox_payload: true }
}
// --------------------------------------------------------------------------------
          ...
          <div style={{ maxHeight: 430, overflow: 'auto' }}>
            {activeStops.map(stop => (
              <div key={stop.pickupItemId} style={{ display: 'grid', gridTemplateColumns: '46px minmax(0, 1fr) 90px', gap: 10, padding: 12, borderBottom: `1px solid ${C.border}66`, alignItems: 'center' }}>
                ...
                <div style={{ color: C.info, fontSize: 12, textAlign: 'right' }}>{(stop.distanceFromHeadOfficeKm ?? 0).toFixed(2)} km</div>
              </div>
            ))}
