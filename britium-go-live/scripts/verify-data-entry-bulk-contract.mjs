...
const checks=[
  ...
  ["Calculate All uses a bounded six-worker queue",/Math\.min\(6,total\)/.test(page) && /Calculating parcels: \$\{completed\}\/\$\{total\} completed/.test(page)],
  ["Calculate All commits collected results once",/const results=new Map<number/.test(page) && /setRows\(current=>current\.map/.test(page) && /edited during bulk calculation/.test(page)],
  ["Calculate All throttles progress rendering",/completed===total\|\|completed%6===0/.test(page)],
  ["individual calculations recover after a timeout",/Promise\.race\(\[/.test(page) && /Calculation timed out after 30 seconds/.test(page) && /setBulkCalculating\(false\)/.test(page)],
  ["Royal rows initialize the Royal tariff filter",/useState\(\(\) => row\.service_provider_code \|\| "ALL"\)/.test(page)],
  ["township routing and tariff matching are memoized",/const route = useMemo/.test(page) && /const matches = useMemo/.test(page) && /const selected = useMemo/.test(page)],
  ["Calculate All reports partial failures",/Calculated \$\{calculated\} of \$\{rows\.length\}/.test(page)],
  ["frontend exposes atomic Save All",/SAVE ALL/.test(page) && /be_data_entry_financial_v2_save_batch_v22/.test(page)],
