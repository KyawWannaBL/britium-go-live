...
if (source.includes("DATA_ENTRY_PERFORMANCE_V40")) {
  console.log("Data Entry Performance V40 already applied");
  process.exit(0);
}

function replaceOnce(label, before, after) {
  if (!source.includes(before)) throw new Error(`V40 patch anchor not found: ${label}`);
...
const locationEnd = source.indexOf('  async function retryImportedLocationSync(){\n');
...
console.log("Applied Data Entry Performance V40 build patch");
