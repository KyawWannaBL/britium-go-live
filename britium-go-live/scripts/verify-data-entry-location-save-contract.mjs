...
const checks=[
  ...
  ["skip review safely resolves a missing pin before audit",/let pin = candidate;[\s\S]*externalCandidate[\s\S]*resolveDeliveryLocation\(\{[\s\S]*deliveryWayId,[\s\S]*address: query \|\| address,[\s\S]*township,[\s\S]*ward,[\s\S]*postalCode,[\s\S]*client: supabase/.test(editor)&&/be_delivery_location_review_batch_v29/.test(editor)],
  ["first-time addresses create a location candidate instead of stopping",/No saved location exists yet\. Creating the first Location Details candidate[\s\S]*void find\(address, true\)/.test(editor)],
  ["bulk review candidate is hydrated into the editor",/externalCandidate=\{row\.locationCandidate\}/.test(page)&&/externalCandidate\?: DeliveryLocation \| null/.test(editor)],
