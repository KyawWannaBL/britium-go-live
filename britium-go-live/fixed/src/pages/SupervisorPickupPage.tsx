...
function TeamStatusCard({label,name,code,status}: any){
  const current = normalizeAssignmentStatus(status);
...
export default function SupervisorPickupPage() {
  ...
  async function confirmAssignment() {
    ...
    // Resolve the actual codes from the dropdown selections
    const riderCode = riders.find(r => workerValue(r) === selectedRider)?.code || selectedRider;
    const driverCode = selectedDriver ? (drivers.find(d => workerValue(d) === selectedDriver)?.code || selectedDriver) : null;
    const helperCode = selectedHelper ? (helpers.find(h => workerValue(h) === selectedHelper)?.code || selectedHelper) : null;
    const vehiclePlate = selectedFleet ? (fleets.find(f => f.id === selectedFleet)?.vehicle_no || selectedFleet) : null;
  ...
  return (
    <div className="space-y-6 overflow-x-hidden">
        ...
        <div className="bg-[#0b2236] border border-[#1a3a5c] rounded-2xl flex flex-col min-h-[560px] min-w-0">
          ...
          <div className="flex-1 overflow-auto custom-scrollbar p-4 space-y-3">
            {filteredQueue.length === 0 ? (
              ...
              const assigned = item.supervisor_status === "ASSIGNED" || item.pickup_status === "RIDER_ASSIGNED" || item.pickup_status === "IN_TRANSIT" || item.pickup_status === "assigned";
              return (
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        <TeamStatusCard label="Rider" name={item.assigned_rider_name} code={item.assigned_rider_code} status={item.rider_status} />
        <TeamStatusCard label="Driver" name={item.assigned_driver_name} code={item.assigned_driver_code} status={item.driver_status} />
        <TeamStatusCard label="Helper" name={item.assigned_helper_name} code={item.assigned_helper_code} status={item.helper_status} />
      </div>
                </button>
              );
            })}
