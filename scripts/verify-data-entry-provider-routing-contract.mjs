...
try {
  ...
  assert.equal(providerFor("", "No. 12, Chanayethazan Township, Mandalay"), "DK DELIVERY");
  assert.equal(providerFor("", "Naypyitaw State Academy, Nay Pyi Taw"), "NPT BRANCH");
  assert.equal(providerFor("", "Monywa Township, Sagaing Region", { fallbackUnknownToRoyal: true }), "ROYAL EXPRESS");
  assert.equal(
    resolveDataEntryServiceProvider("", "Naypyitaw State Academy, Nay Pyi Taw", tariffOptions).township,
    "Naypyitaw",
  );
