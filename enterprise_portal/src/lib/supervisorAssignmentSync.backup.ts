...
export function normalizePickup(row: Record<string, any>): PickupOrder {
  ...
  return {
    ...
    cod_amount: num(row.cod_amount || row.cod_due || row.cod_expected || row.amount, 0),
...
export async function loadLiveGpsRows(): Promise<LiveGpsPoint[]> {
  ...
  return dedupeBy(
    rows.map((row) => ({
      id: text(row.id || row.pickup_id || row.request_code || row.rider_email || row.rider_code),
      ...
      rider_email: text(row.rider_email || row.assigned_rider_email || row.email || row.user_email),
      rider_code: text(row.rider_code || row.assigned_rider_code || row.workforce_code || row.worker_code),
      rider_name: text(row.rider_name || row.name || row.full_name || row.display_name || row.rider_email),
    ...
    (row) => `${row.pickup_id}:${row.rider_email || row.rider_code}:${row.lat || ""}:${row.lng || ""}`,
  );
}
