...
export default function BulkTemplateWorkbench({ module, titleEn, titleMm, subtitleEn, subtitleMm }: Props) {
  ...
  async function enrichRowsWithDeliveryCharges(cleanRows: RowObject[]) {
    const enriched: RowObject[] = [];
    ...
    for (const row of cleanRows) {
      ...
      try {
        const { data, error } = await (supabase as any).rpc("be_calculate_delivery_charge", { p_payload: chargePayload });
        if (!error && data && data.ok !== false) {
          enriched.push({
            ...row,
            delivery_charge: data.total_charge ?? data.delivery_charge ?? row.delivery_charge ?? "",
            total_charge: data.total_charge ?? row.total_charge ?? "",
            tariff_source: data.source ?? "backend_tariff",
            tariff_payload: data,
          });
          continue;
        }
      } catch {
        // Keep original row if tariff RPC is unavailable; backend staging still works.
      }
  ...
  async function saveBulkLoad() {
    ...
    try {
      ...
      const enrichedRows = await enrichRowsWithDeliveryCharges(cleanRows);
    ...
    <div className="be-page btw-page">
      <style>{`
        ...
        .btw-control {
          ...
          background: rgba(30, 44, 68, 0.88) !important;
          color: #f8fafc !important;
          padding: 0 14px;
          outline: none;
