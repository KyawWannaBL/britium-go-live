...
export default function BulkTemplateWorkbench({ module, titleEn, titleMm, subtitleEn, subtitleMm }: Props) {
  ...
  function applyProcessContextToRows() {
    ...
    setMessage(translate(lang, "Selected process context was applied to the webform rows.", "Selected process context was applied to the webform rows."));
  }
  ...
  async function saveBulkLoad() {
    ...
    try {
      ...
      setMessage(translate(lang, "Bulk load saved", "Bulk load saved") + ": " + (data?.batch_id || "OK"));
    } catch (err: any) {
    ...
    <div className="be-page">
      ...
      <section className="be-panel" style={{ marginBottom: 18 }}>
        ...
        <div className="be-card-grid">
          <label><b>{translate(lang, "Inbound Data Source", "Inbound Data Source")}</b><select value={source} onChange={(e) => setSource(e.target.value)}>{DATA_SOURCES.map((item) => <option key={item.id} value={item.id}>{translate(lang, item.en, item.mm)}</option>)}</select></label>
          <label><b>{translate(lang, "Template Format", "Template Format")}</b><select value={template.id} onChange={(e) => setTemplateId(e.target.value)}>{templates.map((item) => <option key={item.id} value={item.id}>{translate(lang, item.titleEn, item.titleMm)}</option>)}</select></label>
          <label><b>{translate(lang, "Branch / Warehouse", "Branch / Warehouse")}</b><select value={branch} onChange={(e) => setBranch(e.target.value)}>{BRANCHES.map((b) => <option key={b} value={b}>{b}</option>)}</select></label>
          ...
          <label><b>{translate(lang, "Date", "Date")}</b><input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label>
        </div>
      </section>
      ...
      <section className="be-panel" style={{ marginBottom: 18 }}>
        ...
        <div className="be-card-grid">
          <label><b>{translate(lang, "Pickup ID", "Pickup ID")}</b><select value={pickupId} onChange={(e) => onSelectPickup(e.target.value)}><option value="">Manual / No pickup selected</option>{pickups.map((p) => <option key={pickupIdOf(p)} value={pickupIdOf(p)}>{displayPickup(p)}</option>)}</select></label>
          <label><b>{translate(lang, "Way ID", "Way ID")}</b><input value={wayId} onChange={(e) => setWayId(e.target.value)} placeholder="Way ID" /></label>
          <label><b>{translate(lang, "Merchant Code / Name", "Merchant Code / Name")}</b><input value={merchantCode} onChange={(e) => setMerchantCode(e.target.value)} placeholder="Merchant" /></label>
          <label><b>{translate(lang, "Process Action", "Process Action")}</b><select value={actionCode} onChange={(e) => setActionCode(e.target.value)}><option value="">Select action</option>{actions.map((a) => <option key={a.action_code} value={a.action_code}>{a.action_code}</option>)}</select></label>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
          <button type="button" onClick={loadPickups}>{translate(lang, "Refresh Pickup List", "Refresh Pickup List")}</button>
          <button type="button" onClick={applyProcessContextToRows}>{translate(lang, "Apply Pickup Context to Rows", "Apply Pickup Context to Rows")}</button>
        </div>
      </section>
      ...
      <section className="be-panel" style={{ marginBottom: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button type="button" onClick={() => buildXlsx(template, blankRows(template.headers, 20))}>{translate(lang, "Download XLSX Template", "Download XLSX Template")}</button>
            <button type="button" onClick={() => downloadBlob(template.id + ".csv", toCsv(template.headers, blankRows(template.headers, 20)), "text/csv;charset=utf-8")}>{translate(lang, "Download CSV", "Download CSV")}</button>
      ...
      <section className="be-panel">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button type="button" onClick={addRow}>{translate(lang, "Add Row", "Add Row")}</button>
            <button type="button" disabled={busy} onClick={saveBulkLoad}>{busy ? translate(lang, "Saving...", "Saving...") : translate(lang, "Save Bulk Load", "Save Bulk Load")}</button>
          </div>
        </div>
